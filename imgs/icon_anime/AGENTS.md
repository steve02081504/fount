...
- Terrain is **pedestal-anchored**; ungrown base keeps `HORIZON` until `POOL`/`SLOPE_*` overwrite. Resize shifts retained dynamics with the icon. New/expanded soil stays dry (no ambient `SOIL_CAP` fill — that would spring under inverted ĝ). Land occupancy is `world.land` (alias `terrain.solid`); melt solidify/melt writes that buffer — no parallel “new crust” store.
...
- Composition bottom (pedestal / lava edge) never rains — even when inverted ĝ makes it a physical sky; quiet until sink-edge exposure yields lava. Any ĝ into the bottom (handheld upside-down always has side tilt) zeroes **all** rain weights — side rain alone would mint water forever under Infinity `rainUntil`.
