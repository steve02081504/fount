...
| `session.mjs` | Process singleton: user-abort `signal`/`abort`, `intro` / `start`+`dismiss` / `farewell` / `sleep`; starts/stops device gravity |
| `gravity.mjs` | Device gravity via `node:child_process` → `termux-sensor` (Termux); default screen-down elsewhere; continuous vector + 4-axis quantize |
| `icon.mjs` | Packed silhouette, pillars, body growth order (typed arrays) |
| `scene.mjs` | Anim state, materials, gravity-aware rain edges, pool leak, enter/hold/exit, resize + melt/temp carry |
| `compose.mjs` | Frame paint + ANSI; lava temperature palette + bubble glyphs; pointer torch/ripples |
...
| `fluid/` | Particles, grid liquid/melt, soil, thermal, boundary, bubbles, Boyle air, gas wind, glyphs |
...
| `mat.mjs` | `MAT` enum, density language (`rhoOf` / `viscOf`), thermal thresholds, soil/liquid constants |
| `flow.mjs` | Torricelli / sheet / hydraulic φ / mass-transfer; `viscGain` scales flow |
| `world.mjs` | Grid alloc (`liq`/`melt`/`temp`), gravity helpers, scratch, `totalWorldWater` |
| `edges.mjs` | Gravity-relative wrap / up-down edge geometry (no side effects) |
| `boundary.mjs` | Down-edge lava source (13s onset), up-edge absorb/regurgitate, side wrap re-exports |
| `thermal.mjs` | Conduction, evaporate/steam, melt/solidify phase change |
| `bubbles.mjs` | Sealed air in melt rises along −ĝ and ruptures at free surface |
| `gas.mjs` | Air regions, hydrostatic open `P_ATM+ATM_HYDRO·depth`, wind, Bernoulli ΔP `stepGas` |
| `liquid.mjs` | Water + melt transport, buoyancy swap, graph-hydraulic equalize |
| `soil.mjs` | Moisture / condensation / Matthew / drip `stepSoil` |
| `particles.mjs` | SoA rain/splash + vector gravity + side wrap; expire deposits mass |
| `step.mjs` | `stepFluid` — label → gas → lift → thermal → particles → liquid → bubbles → boundary |
| `glyphs.mjs` | `waterChar` / `liquidChar` / `dripChar` / `lavaChar` |

## Layout & hot-path notes

- Per frame: `stepFluid` (label-if-dirty → gas → lift → thermal → rain inject → particles → liquid → bubbles → boundary). `labelAirRegions` runs only when `world.airDirty` (mat change or free-liquid/melt crossing `LIQ_DRAW`); `stepLiquid` re-labels mid-tick if particles / lift dirtied topology again.
...
- Open-air thermo in `stepGas` / `pressureAt` is closed-form `P_ATM+ATM_HYDRO·depth` where `depth = gravityDepth` (default = `y`).
...
| empty (`AIR`) | Atmosphere / caves / melt cells | Air when empty; melt mass lives in `melt`/`temp` on AIR cells. Gas velocity lives here. |
| melt (field) | Lava | Same rock continuum as soil; rendered red/orange by `temp`; viscosity from `viscOf(rhoOf(ROCK,temp))`. Bubbles = sealed air regions inside melt. |
...
Falling water (rain particles **and** free liquid) uses `waterChar` from **amount × liquid/droplet velocity** (never gas wind). Lava uses `lavaChar` (thick glyphs when cool). High momentum: `/` `∕` · `\` `∖` · `-`. Low momentum diagonal: `‚´′…` / `‵‛…`. Pure fall: `|¦‖⁞⁚⁝.`. Still pools: `‥…~⁓–`. Grid liquid velocity is `liqVx`/`liqVy` from mass transfers; particles use `particles.vx[i]`/`particles.vy[i]`.

Compose priority (top wins): splash/rain particles → soft icon edges (`.` / `..`) → body-pool `@` / **lava** / free-liquid water glyphs → hanging drip under soil → pillars `:` → terrain outline.
...
- Resize is pedestal-relative: retained cells and dynamics (including `melt`/`temp`) shift intact with the icon; shrinking crops, while expansion generates only exposed cells from the persistent seed. New soil starts saturated; temps BFS-decay from retained melt so lava lakes extend without cold cliffs. `RESIZE_WEATHER_TICKS` includes thermal + liquid settle.

## Physics invariants

- **One pressure language**: gas thermo `pressureAt`, liquid hydro `liquidPressureAt`, gas dynamic `staticPressureAt = P − ½ρu²`. Free-liquid mass moves via Torricelli `pressureMove` / free-surface `sheetMove` (`flow.mjs`), scaled by `viscGain(visc)`.
- **One density language**: `rhoOf(substance, temp)` → `viscOf(rho)`. Gas is the low-rho end; water mid; rock/lava continuum — hotter → lighter → thinner. Cold rock = soil (`MAT.SOLID`/`HORIZON` are cache tags flipped at `T_LIQUIDUS`/`T_SOLIDUS`).
- Open air regions: region mean `pressure = P_ATM`; cell `pressureAt = P_ATM + ATM_HYDRO·depth` (down along quantized gravity → P↑).
- Sealed regions: Boyle mean `pressure ≈ gasAmount / airCells` (isothermal ideal gas at fixed T) plus hydrostatic `ATM_HYDRO·(depth − depthMean)` so the spatial average stays Boyle; gas mass transfers by cell overlap when topology splits/merges. Evaporation injects steam mass into the local region.
...
- Melt shares the same transport primitives with per-cell viscosity; buoyancy swaps along gravity when the downslope neighbor is lighter (drives convection + bubble rise).
...
- Communicating vessels: free surfaces of the same liquid component relax toward equal `φ = P/(ρg) - depth` **along the liquid graph** (BFS from the lowest-φ surface — no teleport across disconnected air).
...
Coverage: deterministic terrain glyphs/features, pedestal land shoulders, icon crust + caves below, tall-land quota (≥30% view cols ≥¼ screen), sealed-cavity compression (Boyle) + sealed hydrostatic stratification, U-tube leveling (graph φ), BODY splash (no flood), pillar pass-through, pool→slab/ground leak, `SEAL` impermeable fixtures, soil absorb (dry>wet) / seepage / Matthew condense / drip + mass conservation, sustained-rain surface puddles, open-air hydrostatic P(depth), liquid column depth pressure + Torricelli √(ΔP) orifice drain, sealed gas pushback on liquid, gas wind time-variation + height shear, wind-tunnel continuity + Bernoulli static-P drop + ΔP suction drive, wall stagnation, particle gas drag + life-expire deposit, water glyphs by amount/liquid-velocity (still puddles vs high/low momentum slant), exit frame bound, resize terrain/dynamics preservation + new-soil weathering, ANSI frame size, gravity sensor mapping + rain edge weights, lava onset/T_MAX rim, thermal melt/solidify/evaporate, up-edge absorb/regurgitate, side wrap, rho/visc monotonicity, buoyancy.
