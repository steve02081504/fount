...
- Liquid settle orders cells deep→shallow by projected depth (counting-sort buckets). Shared `fillCellDepths` / `buildDepthOrder` across pressure fill, water settle, melt transport, buoyancy. `fillCellDepths` caches on gravity basis (gx/gy/depth0/size) so tick-internal callers reuse one WH write; `stepLiquid` builds one deep→shallow order shared by lava + buoyancy. `ensureThermoPressure` fills a `thermoP` table after air label (invalidated on re-label / sealed steam / depth change); `pressureAt` and melt transport hit the table. Pressure cache: dirty seeds coalesce and flush lazily on next `pAt` (DDA lines, or full `fillPressureByDepth` past a seed threshold).
...
- Soil drip + hanging reabsorb share one underside scan (`weightSum > 0` ≡ open air below).
- Thermal conduction swaps `temp` ↔ `thermNextT` (no WH `.set` copy). `strongestUp/Down` accept optional precomputed weights.
