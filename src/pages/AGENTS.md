...
JavaScript helper functions located in `@src/pages/scripts/`.
...
- `async getAllCachedPartDetails(partType)`: Fetches all cached details for a
  given part type.
- `async setDefaultPart(parttype, partname)`: Sets the user's default part for a
  given type.
...
- `usingTemplates(path)`: Sets the base path for your template files (e.g.,
  `/shells/chat/src/public/templates`).
