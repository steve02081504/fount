 ...
 *
 * 用法：
 *   deno run -A ./src/scripts/checks/tools/scan_jsdoc_english.mjs
 *   deno run -A ./src/scripts/checks/tools/scan_jsdoc_english.mjs imgs/icon_anime
 */
import { REPO_ROOT } from '../../test/core/repo_root.mjs'
import { scanJsdocEnglish } from '../jsdoc_english.mjs'

const under = Deno.args[0] || ''
const { files, issues } = await scanJsdocEnglish(REPO_ROOT, under ? { under } : {})
