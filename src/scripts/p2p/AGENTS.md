...
- **P2P identity & node data**: singleton node dir `{dataPath}/p2p/node/` — `node.json` (transport, mailbox routing, `batterySaver`), `network.json`, `denylist.json`, `reputation.json`; per-user operator keypair in `{userDict}/settings/operator.json`; entity profile `{userDict}/entities/{entityHash}/profile.json`. HTTP `/api/p2p/federation` aggregates node transport + operator pubkey (no single on-disk `federation.json`). Other routes: `/api/p2p/network`, `/api/p2p/denylist`, `/api/p2p/entities/*`, `/api/p2p/viewer` (`src/server/web_server/p2p_endpoints.mjs`). Does not depend on shell Load.
- **TrustGraph fanout**: Social timeline / chunk exploration go through `fanoutToTopNodes`; **targeted business packets** (Mailbox) use `sendToNode` / User Room, never fanout. Group rooms are injected via `registerFederationRoomProvider` (registered by Chat Load); the P2P layer does not import Chat. User room password is `sha256('fount-user-room:' + nodeHash)`, serving as a global Public Inbox. WebRTC rendezvous uses Trystero **Nostr** strategy (`signaling_room.mjs`) with multi-relay redundancy (trystero defaults ∪ fount curated ∪ user `relayUrls`).
