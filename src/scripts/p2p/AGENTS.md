...
- **TrustGraph fanout**: Social timeline / chunk exploration go through `fanoutToTopNodes`; **targeted business packets** (Mailbox, `deliver`) use only `sendToNode` / User Room, never fanout. Group rooms are injected via `registerFederationRoomProvider` (registered by Chat Load); the P2P layer does not import Chat. User room password is `sha256('fount-user-room:' + nodeHash)`, serving as a global Public Inbox. WebRTC rendezvous uses Trystero **Nostr** strategy (`signaling_room.mjs`) with multi-relay redundancy (trystero defaults ∪ fount curated ∪ user `relayUrls`).
