...
- **Discovery relay override**: live tests inject shared loopback relays via `initNode({ signaling: { relayOverride, mdnsPolicy, trickleIceOff } })` in the test harness (`src/scripts/test/node/p2p_signaling.mjs` + `--p2p-relay-url` on the node worker). Honor `getSignalingRuntimeConfig().relayOverride` in all discovery paths.
