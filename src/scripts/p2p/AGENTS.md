...
- **Link establishment (connId dual-PC pick-one)**: both peers dial directly (zero extra cost for one-way dial); on a true simultaneous dial each builds a PC and `registerResolvedLink` deterministically drops one, keeping the link initiated by the smaller nodeHash. Signal frames carry `connId` (`signalSessions` indexed by connId); an inbound offer builds an independent answer PC, not blocked by the per-nodeHash `inflights`. `link.mjs` is a stateless dumb pipe — no rollback/perfect-negotiation (the polyfill has no rollback).
- **Sparse group linking**: `group_link_set` is not a full mesh; `selectLinkTargetsFromMembers` (`peer_pool`) picks top-K trusted + random explore + mandatory initial-member anchors within the `resolveFederationPoolLimits` budget; recomputed (debounced) on membership change; never proactively cuts (`trimToBudget` backstop).
- **dag_event first-seen relay**: `roomHandlers/sync.mjs` forwards the stripped event to `pickFederationTargetPeerIds` (minus sender) on first sight (`tryMarkSeenFederationEvent`) with a passing signature (`ingestRemoteEvent` state ∈ {applied, pending, quarantined}; invalid is not forwarded), to speed DAG convergence on sparse meshes; no reputation penalty for relaying.
- **Signaling edge cases** (connId dual-PC / glare, sparse linking, dag_event relay, Windows ICE, live-test relay override): [docs/signaling.md](docs/signaling.md).
