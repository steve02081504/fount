...
function New-FedNodeHandle($index) {
	...
	if (-not ($base -and $base.Trim())) { throw "${baseKey} is required; run via test/live/run.mjs" }
	if (-not ($key -and $key.Trim())) { throw "${keyKey} is required; run via test/live/run.mjs" }
