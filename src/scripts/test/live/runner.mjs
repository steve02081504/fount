...
function buildLiveNodeEnv(nodes, repoRoot) {
	...
	const env = {
		...
		FOUNT_NODE_A_DATA: nodes[0].dataPath,
	...
	for (let i = 0; i < nodes.length; i++) {
		...
		if (idx === 1) 
			env.FOUNT_NODE_1_DATA = node.dataPath
		
		else if (idx === 2) {
			env.FOUNT_NODE_B_DATA = node.dataPath
			env.FOUNT_TEST_NODE_B_BASE_URL = node.baseUrl
			env.FOUNT_TEST_NODE_B_PORT = String(node.port)
			env.FOUNT_TEST_NODE_B_KEY = node.apiKey
		}
		else 
			env[`FOUNT_NODE_${idx}_DATA`] = node.dataPath
