...
- **Schedule / ETA**: the kernel keeps an **ideal scheduling plan** (`kernel/schedule.mjs`): it feeds the live queues + running + budget into `buildTimeline`, which simulates the `ResourceRunGate` admission rules (heavy exclusive, light 2-D packing, deps-first, module-check mutex) into absolute time slots. On any state change it broadcasts a **`schedule-update`** event (queue/enqueue/promote, gate acquire/release, suite start/end/fail/blocked/skip, budget refresh) with per-consumer projection (`kernel/schedule_event.mjs`): running items + `lastCompletionMs` (the consumer's last task completion, accounting global contention) + a `reason`. `prep` (3-min debounce) participates only after `promotePrep`. Consumers (`display/schedule.mjs`) render on a **5% change vs last shown**, printing the total-remaining and the reason. Budget refreshes every `BUDGET_REFRESH_MS` (5s) with `BUDGET_CHANGE_THRESHOLD` (10%) hysteresis; the old `clampRemainingMs` monotonic clamp is gone — the plan is rebuilt from real state each time. `prep` participates only after promotion. Incrementally, the kernel auto-rewrites `expected` (suite/subtest) when it drifts beyond the scale-dependent tolerance `expectedDriftToleranceMs` ≈ `37·scale^0.656` ms (relative to the larger; missing value with a baseline counts as drift) — same-manifest writes serialized, in-memory `expectedMs` synced, no full reload. Disable via `autoUpdateExpected: false`.
