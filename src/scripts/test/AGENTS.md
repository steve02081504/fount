...
- **Fixture probes（同进程共享状态）**：char/world/persona fixture 虽被 `cp` 到临时 `users/…`，仍应通过 `fount/public/parts/shells/…/test/fixtures/probes/*.mjs` 导入**模块级单例**（`onMessageProbe` / `groupEventProbe` / `socialOnMessageProbe` 等），由测试与 fixture 共享同一实例。**禁止** `globalThis.__fount*`。元数据占位 origin 勿写生产口（如 live `bridgeOrigin` 用 `http://live-bridge.test`，不是 `127.0.0.1:8931`）。
