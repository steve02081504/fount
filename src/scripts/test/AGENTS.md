...
- **`FOUNT_TEST_TRIGGERED_FILES`**: absolute path to a temp file listing repo-relative paths that matched the suite's triggers for this wave (committed since last record ∪ uncommitted). Empty = unconstrained (child does its full default). Runner writes the file under a per-suite tmp dir and deletes it after the suite ends. Checks like `json_lf` scope to those paths when the list contains matching suffixes; otherwise fall back to a full scan. Protocol: [protocol.mjs](core/protocol.mjs).
...
- **Chat / Social fixtures**: `createCharBoot` / `seedCharFixture` / `seedStubCharPart` (main.mjs stub only — for `resolveCharPartName` / `ensureAgent*`) / `waitUntil` from `shells/chat/test/harness.mjs`; Social agents: `seedAgentChar` in `shells/social/test/harness.mjs`. Char part names are aligned to real `chars/` directory names via `resolveCharPartName` at write boundaries; Hub compares with exact `===` — do not fold.
