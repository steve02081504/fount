...
- **Subtests**: suite may register `subtests: [{ name, trigger/triggers, spec? }]` in manifest. Suite-level `trigger` = shared infra (hit → all subtests stale); each subtest has its own triggers + state fingerprint. Selector: `manifest:suite:subtest` / `manifest/suite/subtest`. Runtime filter: `FOUNT_TEST_SUBTESTS`. Plan slots stay suite-scoped but carry `subtestsToRun`.
...
- **`subtests`**: optional per-suite list; each entry `{ name, trigger|triggers, spec? }` (`spec` defaults to `${name}.spec.mjs`). Used by frontend Playwright suites so editing `feed.mjs` only expires `frontend:feed`.
