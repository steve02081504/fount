...
- **`skip_because`**: GitHub issue URL, `{ url, delay }`, or an array of those on suite or subtest. Still open / `gh` fail / closed but within `delay` → skip as pass. Closed and `now >= closedAt + delay` (or closed with no `closedAt` when delay is set) → fail and list URLs to follow up. `delay` uses the same duration syntax as `expected` (`14d` / `4m12s`); a number is milliseconds; omit for 0. Same URL keeps the larger delay. Never reuse via fingerprint.
