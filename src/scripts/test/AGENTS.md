...
- **ETA monotonic**: displayed remaining only decreases at wall-clock rate unless the queue actually grows (`cli`/`fs`/`prep`); `idle_all` never counts as insertion. Debug per-snapshot makespan with `FOUNT_TEST_DEBUG_ESTIMATE=1` → `debug_logs/test_estimate.log`. Incrementally, the kernel auto-rewrites `expected` (suite/subtest) when it drifts >10% from the state baseline (relative to the larger; missing value with a baseline counts as drift) — same-manifest writes serialized, in-memory `expectedMs` synced, no full reload. Disable via `autoUpdateExpected: false`.
