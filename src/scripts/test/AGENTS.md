...
- **Browser scripts**: `/scripts/*` → `src/public/pages/scripts/`. Chat/Social shared: `shells/*/public/shared/` or `/parts/shells:…/shared/*` — Deno-pure-importable, no `/scripts` or `/parts/` URL imports. **Trap**: relative climbs from part `public/` to `pages/scripts` resolve in the browser as `/pages/scripts/…` (404); browser-only modules use absolute `/scripts/…`. Do not co-locate DOM/`/scripts` helpers with pure exports so a `test/pure` file can import the same module — split: pure → `shared/`, UI → `public/src/`.
