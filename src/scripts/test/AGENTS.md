...
- **`expected`**: duration seed (`16s` / `4m12s` / ms number) for ETA when state has no baseline. Suite = full-run wall; subtest = that spec. Refresh with `fount test --update-estimates`. Incrementally, the kernel auto-rewrites `expected` (suite/subtest) when it drifts >10% from the state baseline (relative to the larger; missing value with a baseline counts as drift) — same-manifest writes serialized, in-memory `expectedMs` synced, no full reload. Disable via `autoUpdateExpected: false`.
