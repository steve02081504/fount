...
- **GitHub issue probe**: `core/github_issue.mjs` → `parseGithubIssueUrl` (pure). Closed-state via `hub/apis/github_issue.mjs` (`gh` + `AbortSignal.timeout`) + `hub/clients/github_issue.mjs` / browser page `watch` (bounded fetch timeout → treat as open). Playwright `assertAriaIgnoreIssues` hard-fail closed `[aria-ignore]` URLs. No hub / `gh` down → still open. Selftest suite: `testkit:test_hub`.
