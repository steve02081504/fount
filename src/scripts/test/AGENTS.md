...
- **Default loop** (bare `fount test`): imperfect wave (`failed`/`blocked`/missing + one-level dependents; **not** fresh `noisy`) → hard fail (`failed`/`blocked`/pending) exits 1; else outdated wave (`unknown`) → back to imperfect; both empty → exit 1 if scope still has fresh noisy (message lists them), else 0 with “all green”. Never full-repo unless `--all`. No auto-retry of failures within one invocation.
...
- **Browser scripts**: `/scripts/*` → `src/public/pages/scripts/`. Chat/Social shared: `shells/chat/public/shared/` or `/parts/shells:chat/shared/*`. Do not import `/scripts/test/*` from Deno-only trees. **Trap**: relative climbs from part `public/` to `pages/scripts` resolve in the browser as `/pages/scripts/…` (404). Browser-only modules must use absolute `/scripts/…`; modules also imported by Deno pure tests must not depend on that path (inline or use relative imports on the test side).
