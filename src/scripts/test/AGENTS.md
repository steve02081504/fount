...
- **Output modes**: `human` (TTY dashboard) / `plain` / `json` (NDJSON), via `FOUNT_TEST_OUTPUT` or `--output`/`--json`; non-TTY defaults to `plain`. `plain`/`json` drop the schedule ETA/reason stream (the repeated `remaining: N · unknown duration: 0` flood) and instead emit discrete events (accepted / suite start–end / job-wait / job-done) plus a 30s heartbeat. `display/output.mjs`.
