...
- **Browser scripts**: paths under `/scripts/*` map to `src/public/pages/scripts/` (e.g. `/scripts/test/ready_gate.mjs` → `public/pages/scripts/test/ready_gate.mjs`; Chat/Social P2P 与 mention/avatar 原语走 `shells/chat/public/shared/` 或 `/parts/shells:chat/shared/*`。 Do not import `/scripts/test/*` from Deno-only trees.
...
- **Naming**: prefer readable identifiers in framework code — `context` not `ctx`, `core/dependencies.mjs` not `deps.mjs`, `dependencyEdges` not `deps`. Suite / file / fixture / `Deno.test` 名与注释用领域语义，勿带规划里程碑代号（`M1`/`M7a`/`G4` 等）；规划只在设计 md。
