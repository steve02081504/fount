...
Per-slot `continueReason` in `data/test/report.json` and `data/test/triggered-reasons.md` (linked from `report.md`) explain why each suite was included. `triggered-reasons.md` is written **once** when the report is created (wave enqueued/planned) — the reasons are fixed at plan time and don't change as the run progresses — so it is not rewritten on every `report.md` update. Implementation: `runner/continue_reason.mjs` from the plan built by `core/plan.mjs`.

**CI** caches `data/test` as `fount-test-data-<branch>`; restore prefers exact key, else nearest tip-by-`git diff --name-only` among listed caches (tie → default branch → newer `createdAt`). Strips logs/tmp/playwright/heapsnapshots/report before save. Failed/noisy suite tails are printed once at `job-done` — do not rely on cached log files in CI. Merged PR that was fully ahead of base (same tree as head) → `promote_test_data_cache` copies head→base before branch-delete cleanup and before default-branch Run Tests restore.
...
- A goal pulled in as `imperfect_dependent` (one-level downstream of a hard-fail root) always **runs**, never reuses — even when its own content is unchanged. It must not carry a stale green forward while its parent is being re-validated; the runtime then either commits it (parent passed) or records it `blocked` (parent failed). `skip_because` (URL / `{url, delay, as}` / array) never reuses via fingerprint; skip-pass does not stamp a fresh green record. Default `as: "pass"` counts as green for planning (leftover failed is not imperfect and does not block dependents; this session still unblocks via `sessionPassed`). `as: "skip_tree"` omits transitive dependents (`skipped`, not blocked / not a job failure). Closed issues still within `delay` stay skip-pass; closed and past delay fails the slot.
...
| `skip_because` | Re-probe `skip_because` issue URLs |
