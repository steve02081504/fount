...
**Dashboard (`display/dashboard.mjs`)**: when stdout is a TTY with ANSI support, overview / multi / watch replace the running-status lines with a pacman-style bottom status band — a header row (total remaining + running/passed/failed/queued counts) then one row per running suite (name left, per-suite progress bar right, elapsed + projected remaining inline). A finishing suite is committed to the scrollback above the band as `✓/✗ name elapsed … · CPU …% · mem …`, fed by `suite-end.durationMs / avgCpuPct / peakMemMb`. Non-stream output and interleaved messages (queue, job-wait, cleanup-leak) are routed through the band; `job-done` tears the band down before the failure-log replay. Stream mode (live child output) and non-TTY runs keep the plain line output.
