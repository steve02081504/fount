...
`watch` (`scripts/test/watch/`): mounts `fount.test.watch` (`PageWatch`: `kick` / `drain` / `lastRun` / `ready`). Inner `WatchLoop` rotates a11y + locale tasks (no backlog / collision). Task `run({ draining })` returns `true` = idle → next immediately; `false` = did work → wait `delayMs`. Full idle round parks until `wake`. Drain calls each task's `beginDrain` then runs until all `covered()`. (1) `A11yWatch` — MutationObserver dirty → axe; `ARIA_IGNORE` + hub issue probes. (2) `LocaleWatch` — zh-CN → ja-JP → en-UK (`localeHold` skips); script scan hides `[user-content]`. Playwright teardown `waitForLocaleCycle` → `watch.drain()`. Hard-fail on `[test:a11y]` / `[test:locale]` except axe `color-contrast` and `link-in-text-block`. Prefer `[data-i18n="…"]` selectors over locale-specific visible text. UI chrome must use `data-i18n` / `setLocalizeLogic` so locale rotation does not leave stale `geti18n()` strings.
