...
- svgTheme: when a visible `<svg>` is present, sweep `data-theme=light` / `data-theme=dark` and assert each SVG foreground (fill/stroke/currentColor) stays at OKLab ΔE >= threshold from its backing background; an icon that vanishes under one theme fails via `[test:svg]`. Measurement runs inside an `ignore()` block with transitions/animations disabled, then restores the theme.
