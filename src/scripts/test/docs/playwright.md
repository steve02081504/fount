...
- cssvar: scan same-origin `<link>` stylesheets (skip injected Tailwind `<style>` / CDN daisyUI) for CSS 变量健康 — (1) `var(--x)` 无 fallback 且任何元素都无法解析 → 补定义或移除用法; (2) `--x:` 声明但测试期间从未被 `var()` 引用 → 拓展测试覆盖用例或移除死变量. 只认同源变量，避免第三方库误报.
