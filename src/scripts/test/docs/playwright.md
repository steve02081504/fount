...
test('secure render keeps spoiler onclick', async ({ modulePage }) => {
	const html = await modulePage.run(async () => {
		// Expensive objects live on the page-persistent store and are reused across cases (building a processor is costly)
...
- locale: zh-CN → ja-JP → en-UK. Rotation rebuilds DOM (shell `onLanguageChange`), so outside `drain` it only fires after a **quiet window** (no DOM mutation / pointer / keyboard activity for ~1.2s) and while not `holdLocale`-held; teardown `drain` forces the remaining languages. No `[data-i18n]` = textless page (skip rotation; axe also skips `html-has-lang`). Visible-text + **aria-label** scans (skip `[user-content=""]` / `[language-check-ignore]` / `[prompt-content]` / `[aria-hidden="true"]` / `[inert]` / `[hidden]` / `.hidden`; `user-content="aria-label"` skips only that element's own `aria-label`) require Han on zh-CN and Hira/Kata/Han on ja-JP; en-UK must not carry CJK. Use `data-i18n` object keys — never hardcode English `aria-label` as a fallback. `[user-content]` = user/dynamic text; `[language-check-ignore]` = intentional multilingual chrome (language names, EULA in a chosen locale); `[prompt-content]` = prompt/model/plugin content (tool logs, tool cards).
- layout: for each CSS multi-column container (`column-count > 1` or `column-width`), flag any block-level child whose `getClientRects()` spans more than one fragment — a "visually halved" card (e.g. an empty state split across masonry columns) fails via `[test:layout]`. Inline-level children are skipped (line wrapping legitimately yields multiple rects). Fix by `break-inside: avoid` on the child, or dropping to one column while it is present.
- viewport: for a full-viewport app shell declaring `<body data-app-shell>`, flag a document taller than the viewport by less than 10% (a nearly-unscrollable page scrollbar) via `[test:viewport]`. The check appends a hidden `position: absolute` probe — an absolute element contributes to document scrollable overflow — mimicking a browser extension's selection tooltip landing after a `h-dvh` layout; a shell that lets it push the document must clip `body` (`overflow: clip`). Long documents (> 1.1× viewport), viewports that cannot scroll (`overflow: hidden|clip`), and pages without `data-app-shell` are skipped (only exactly-one-screen shells should declare it).
- flicker: sample elements whose `class` / `style` / `hidden` / `open` mutate, once per frame; ≥ 6 visibility flips within 2s reports `[test:flicker]` — catches a "back to bottom" button or class toggle oscillating at a threshold (typical during streaming). Visibility is style-level only (`display` / `visibility` / opacity; a `transform: scaleX(0)` shrink is not flicker); `ignore()` blocks (locale rotation / theme measurement) are skipped.
- Playwright teardown `waitForWatchDrain` → `watch.drain()`.
- Debug tip: to probe a `watch/*` check in isolation (e.g. the SVG `url(#grad)` handling above), a scratch script under Temp launching the system browser directly (`where.exe chrome`/`msedge` + `chromium.launch({ executablePath })`) with a `page.evaluate` replica of the check finds the fault without booting the pages server; the pre-approved scratch dir under `$env:TEMP` is available. Rotation already pauses during interaction (quiet window); an explicit `holdLocale` / `releaseLocale` still forces it off for a block (see `json_editor.mjs`).
- Hard-fail on `[test:a11y]` / `[test:cssvar]` / `[test:emoji]` / `[test:flicker]` / `[test:layout]` / `[test:locale]` / `[test:viewport]` / `[test:watch]` except axe `color-contrast`, `link-in-text-block`, and `html-has-lang` when the document has no `[data-i18n]` (no chrome copy). Prefer `[data-i18n="…"]` selectors. UI chrome must use `data-i18n` / `setLocalizeLogic`. `[test:emoji]` flags emoji in visible chrome text / `aria-label` (Iconify only); user content is exempt via `user-content=""` / `language-check-ignore` / `prompt-content`.
