...
`watch` (`scripts/test/watch/`): mounts `fount.test.watch` (`PageWatch`: `kick` / `drain` / `holdLocale` / `releaseLocale` / `started`). Locale bootstrap then `loop.start()` — the only ready gate (`started` reflects it). Inner `WatchLoop` rotates a11y + locale tasks (no backlog / collision); wake during a tick is deferred to tick end. Task `run({ draining })` returns `true` = idle → next immediately; `false` = did work → wait `delayMs`. Full idle round parks until `wake`. Drain calls each task's `beginDrain` then runs until all `covered()`. (1) `A11yWatch` — MutationObserver dirty → axe; `[aria-ignore]` via shared `test/aria_ignore.mjs` + hub probes. (2) `LocaleWatch` — zh-CN → ja-JP → en-UK (`holdLocale` skips); script scan hides `[user-content]`. Playwright teardown `waitForLocaleCycle` → `watch.drain()`. Pause rotation during asserts with `holdLocale` / `releaseLocale` (see `json_editor.mjs`). Hard-fail on `[test:a11y]` / `[test:locale]` / `[test:watch]` except axe `color-contrast` and `link-in-text-block`. Prefer `[data-i18n="…"]` selectors over locale-specific visible text. UI chrome must use `data-i18n` / `setLocalizeLogic` so locale rotation does not leave stale `geti18n()` strings.
