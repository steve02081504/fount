...
- **`--no-parallel`** — serial gate (`serial: true`): one non-heavy suite at a time, streamed stdout/stderr. Also forces `FOUNT_TEST_BUDGET_CORES=1` so `serial.mjs` inner file parallelism collapses to 1 (avoids Windows `node_modules` lock / `PermissionDenied` races). Default runs buffer per-suite output and print summaries.
...
When `run` includes `serial.mjs`, `suite_run.mjs` injects `FOUNT_TEST_BUDGET_CORES` / `FOUNT_TEST_BUDGET_MEM` from the global budget. Not tied to `heavy`. Under `--no-parallel`, the orchestrator sets `cores = 1` before injection. Silent passes emit `[serial] ok …` for idle watchdog liveness.
