...
- **Frontend join has two entry points — keep them in sync**: `deepLinkConsume` (`applyChatRunUri`, fount://run deep links) and `ensureGroupMembership` (`groupMembership.mjs`, Hub group selection / `?invite=` invite codes). Both must solve PoW when the local replica cannot confirm `joinPolicy` — a local-less replica's `getGroupState` returns the **default** `joinPolicy: 'invite-only'` regardless of the real policy, so checking `state.groupSettings.joinPolicy === 'pow'` alone misses groups that were switched to pow after the invite link was minted (join then fails with "invalid or expired pow solution" since no `powSolution` is attached). Fall back to `getPowChallenge` when `resolvePowForJoin` yields null. Regression: `fed_invite_then_pow` live suite.
...
- **Federation sim registers chat event type defs** (`registerChatEventTypeDefs()` at `simulation/federation.mjs` import). Headless processes never load the chat part, so without it `getGovernanceAuthzTypes()` is empty and `selectConsensusBranchTip` degenerates to tip-id lexicographic tie-breaks on fork — a dangling-parent tip (e.g. `group_settings_update` whose parent was folded away at the source, then federated) can win and fold the real chain away, silently dropping members. Any new headless harness that replays DAG folds must register too; verify with `governanceFork` states, not just row counts.
- **Never statically import the chat session module graph before boot** in a test file (`…/src/chat/session/*.mjs` → `src/chat/session.mjs` runs `initializeGroupMetadatas()` at import, which needs booted `config`). A failed static import registers **0 tests** — the file shows `ok | 0 passed` and the suite silently loses coverage. Dynamic-import such modules inside the test body (post-boot) instead.
