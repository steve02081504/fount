...
function buildIssueBody(panic, version, commitHash) {
	const lines = [
		...
		panic.message ? `- **message**: ${panic.message}` : null,
		panic.platform ? `- **Platform**: ${panic.platform}` : null,
		panic.stackUrl ? `- **Stack trace**: ${panic.stackUrl}` : null,
		panic.args ? `- **Args**: \`${panic.args}\`` : null,
		'',
