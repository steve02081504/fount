 ...
 */
async function* findManifestFiles(dir, repoRoot) {
	let entries
	try {
	...
	for (const entry of entries) {
		...
		yield* findManifestFiles(path, repoRoot)
	}
}
...
export async function loadAllSuites(repoRoot) {
	...
	for await (const manifestPath of findManifestFiles(repoRoot, repoRoot)) {
...
export function resolveManifestSelectors(selectors, knownIds) {
	...
	for (const raw of selectors) {
		...
		for (const pattern of patterns)
			hits = knownIds.filter(id => matchGlob(pattern, id))
