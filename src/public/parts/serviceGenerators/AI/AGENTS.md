...
The proxy supports both Chat Completions and OpenAI Responses API. `config.api_mode` accepts `auto` (default), `chat`, or `responses`; auto prefers the style implied by the configured URL, retries the alternate style on failure, and persists the URL that worked without pinning the mode. Responses requests reuse the Codex Responses client and strip Chat-only model arguments; keep `BuildPrompt` aligned with the selected request shape. Responses is stateless, so multi-turn history must replay prior assistant turns as `output_text` content parts ([codex/src/responsesClient.mjs](codex/src/responsesClient.mjs) `messagesToResponsesBody`), never a bare string — strict backends (vLLM / LiteLLM) validate a typed assistant message as `ResponseOutputMessage` and 400 on a string, iterating it character-by-character. Assistant turns only admit `output_text`/`refusal`, so attachment-carrying assistant history must be downgraded to `user` via `convert_config.forbidAssistantFiles` (same shared policy as Chat Completions, e.g. `["^image/"]` for Kimi); the conversion does not downgrade by default.
...
**`BuildPrompt` (snapshot / cache)**: every source should expose `BuildPrompt(prompt_struct)` → the same outbound-form JSON structure it sends (config-aware: depth, `convert_config`, …) but with binary kept as `Buffer` (never base64) and without hitting the network. It feeds `chat/src/prompt_struct/serializeSnapshot.mjs` (deep tab-text serialization; buffers → variable-length hash) and Agent Studio's prompt-cache estimate. OpenAI-compatible sources get it free via [openaiCompatibleSource.mjs](proxy/src/openaiCompatibleSource.mjs) (`binaryMode: 'buffer'`); the Responses (`createResponsesSource`) / claude-api / gemini factories each provide their own; wrappers delegate via [proxy/src/buildPromptDelegation.mjs](proxy/src/buildPromptDelegation.mjs). Keep it in sync with `StructCall` by sharing one pure conversion helper.

**Multi-round display state**: `base_result` is reused across tool rounds. When a new AI response has no `content_for_show`, remove any value left on `base_result` before merging the new response; an earlier tool call may have set it to `''`, hiding a correct final answer in shells that display `content_for_show ?? content`. Cover both the proxy Responses path and the dedicated Codex Responses source when changing this behavior.
...
**Assistant prefill**: `convert_config.assistantPrefill` (default on) appends a trailing assistant message carrying the role-envelope opener — `<message "uid">\n<sender>Charname</sender>\n<content>` — so the model continues straight from the body (assistant prefill / response prefill). It is implemented once in [proxy/src/assistantPrefill.mjs](proxy/src/assistantPrefill.mjs) (`buildAssistantPrefillEnvelope` / `assistantPrefillEnabled`) and wired into [proxy/src/messageBuilder.mjs](proxy/src/messageBuilder.mjs) (so proxy Chat Completions + Responses, codex, Azure, Cloudflare, Copilot all get it) and [claude-api/main.mjs](claude-api/main.mjs) (so claude-api, claude-oauth, and the Cloudflare Anthropic branch get it). It replaces `roleReminding` (skipped while active) and yields to `forceUserMessageEnding`; `BuildPrompt` mirrors `StructCall`. Never claim it as an OpenAI-standard `prefill` parameter, and do not wire it into sources whose provider does not continue a trailing assistant turn: gemini / vertex, bedrock (model-dependent), cohere / ollama / local, freeuse, and the cookie/web wrappers (grok / duckduckgo / blackbox / notdiamond / claude).
