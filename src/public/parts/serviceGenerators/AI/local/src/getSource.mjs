...
import { buildChatMessages, buildLocalPromptStruct } from './promptBuilder.mjs'
...
export async function GetSource(config) {
	...
	const result = {
		...
		/**
		 * 按本源配置把 prompt_struct 构建成本地 GGUF 会话出站消息（图片不发送，仅以占位说明），供快照与缓存对比。
		 * @param {prompt_struct_t} prompt_struct - 结构化提示。
		 * @returns {Promise<{messages: Array<{role: string, content: string}>}>} 出站结构。
		 */
		BuildPrompt: async prompt_struct => buildLocalPromptStruct(prompt_struct, config),
