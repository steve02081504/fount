...
Deno.test('blackbox BuildPrompt returns envelope messages array', async () => {
	const source = await generator.interfaces.serviceGenerator.GetSource({
		name: 'blackbox-build-prompt',
	...
	assert(chat.content.startsWith('<message'), 'blackbox envelope has no indent')
