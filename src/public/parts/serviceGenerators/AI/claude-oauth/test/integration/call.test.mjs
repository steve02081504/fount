...
Deno.test('Claude OAuth Call sends auth token beta header without billing spoof', async () => {
	...
	try {
		...
		assertEquals(auth?.includes('oauth-token'), true)
		const beta = headers.get('anthropic-beta') || mock.calls[0].init.headers?.['anthropic-beta']
		assertEquals(String(beta).includes('oauth-2025-04-20'), true)
