...
.fount-logprob-token{position:relative;display:inline;vertical-align:baseline;--fount-logprob-confidence-percent:50%;--err:${useThemeStyles ? 'var(--color-error, #ef4444)' : '#ef4444'};--ok:${useThemeStyles ? 'var(--color-success, #22c55e)' : '#22c55e'};--token-underline:color-mix(in oklab,var(--err) calc(100% - var(--fount-logprob-confidence-percent)),var(--ok) var(--fount-logprob-confidence-percent))}
/* theme-radius-ignore */
.fount-logprob-token{border-bottom:2px solid var(--token-underline)}
.fount-logprob-token-value{white-space:pre;opacity:0.95}
.fount-logprob-token-tip{display:none;position:absolute;left:50%;top:-6px;transform:translate(-50%,-100%);z-index:9999;background:${useThemeStyles ? 'var(--color-base-100, #111)' : '#111'};color:${useThemeStyles ? 'var(--color-base-content, #fff)' : '#fff'};border:${useThemeStyles ? 'var(--border)' : '1px'} solid ${useThemeStyles ? 'color-mix(in oklab, var(--color-base-content, #fff) 16%, transparent)' : 'rgba(255,255,255,0.15)'};padding:6px 8px;border-radius:${useThemeStyles ? 'var(--radius-box)' : '8px'};max-width:360px;white-space:pre-wrap;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,'Liberation Mono','Courier New',monospace;font-size:12px;box-shadow:0 4px 18px rgba(0,0,0,0.35)}
...
.fount-logprob-token-tip .fount-logprob-chosen{font-weight:700;background:${useThemeStyles ? 'color-mix(in oklab, var(--color-base-content, #fff) 8%, transparent)' : 'rgba(255,255,255,0.08)'};border-radius:${useThemeStyles ? 'var(--radius-field)' : '4px'};padding-inline:2px}
