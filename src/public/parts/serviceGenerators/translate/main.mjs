	...
	interfaces: {
		parts: {
			...
			loadSubPart: (my_paths, username, partname) => {
				return loadPartBase(username, 'serviceGenerators/translate/' + partname, {}, { skipParentDelegation: true })
			},
			/**
			 * 卸载子部件。
			 * @param {string[]} my_paths - 搜索路径列表。
			...
			unloadSubPart: async (my_paths, username, partname) => {
				return unloadPartBase(username, 'serviceGenerators/translate/' + partname, {}, { skipParentDelegation: true })
			}
		}
	}
}
