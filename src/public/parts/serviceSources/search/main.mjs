...
function GetPath(username, partname) {
	return getUserDictionary(username) + '/serviceSources/search/' + partname
}

/**
 * 加载搜索服务生成器。
 * @param {string} username - 用户名。
 * @param {string} searchSourcename - 搜索服务源名称。
 * @returns {Promise<any>} 加载的生成器实例。
 */
function loadSearchSourceGenerator(username, searchSourcename) {
	return loadPartBase(username, `serviceGenerators/search/${searchSourcename}`)
}
 ...
 */
async function loadSearchSourceFromConfigData(username, data, { SaveConfig }) {
	const generator = await loadSearchSourceGenerator(username, data.generator).catch(e => {
		console.error(e)
		return loadSearchSourceGenerator(username, 'google')
	})
 ...
 * @returns {Promise<any>} 加载的服务源实例。
 */
export async function loadSearchSourceFromNameOrConfigData(username, source, unnamedSources, options) {
	if (Object(source) instanceof String) return loadPartBase(username, 'serviceSources/search/' + source)
	const instance = await loadSearchSourceFromConfigData(username, source, options)
	...
	interfaces: {
		parts: {
			...
			loadSubPart: (my_paths, username, partname) =>
				loadPartBase(username, 'serviceSources/search/' + partname, { username }, { skipParentDelegation: true }),
			/**
			 * 卸载子部件。
			 * @param {string[]} my_paths - 搜索路径列表。
			 * @param {string} username - 用户名。
			...
			unloadSubPart: async (my_paths, username, partname, reason) => {
				return unloadPartBase(username, 'serviceSources/search/' + partname, {}, {
					/**
					 * 获取路径的回调。
					 * @returns {string} 路径。
					 */
		...
		serviceSourceType: {
			loadFromConfigData: loadSearchSourceFromConfigData
		}
	}
}
