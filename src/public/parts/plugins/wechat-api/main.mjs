import { getWechatRuntimeForChar } from '../../shells/wechatbot/src/default_interface.mjs'
...
- wechat_api：门面对象，含 \`ownerWeChatId\`、\`sendText(text)\`、\`sendFiles(files)\`、\`getChatLogs()\`（桥接模式下返回空数组，历史见 chat DAG）
${inWx ? '- wechat_message：本轮触发回复的入站微信消息对象（桥接模式通常不可用）\n' : ''}
常用示例：
