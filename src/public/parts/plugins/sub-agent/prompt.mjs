 ...
 * 【关联】main.mjs 的 interfaces.chat.GetPrompt；state.mjs 的频道/通知注册表；runtime.mjs 创建运行。
 */
import { getRun, registerChannel, takePendingNotifications } from './state.mjs'

/** 子代理工具说明文本（面向模型；标签语法与 handler.mjs 保持一致）。 */
const SUB_AGENT_PROMPT = `\
你可以派生「子代理」来隔离上下文、并行完成复杂任务。子代理复用你的人格与世界设定，但拥有独立对话、独立工作目录与独立预算。
...
- \`plugins\`：子代理可用的插件，逗号分隔。**该列表完全替换默认列表**（默认：code-execution、file-operations、sub-agent、context-compress），不会与你的插件合并；\`sub-agent\` 永远会被强制加入。\`fount_chat\` 永远不会被加载。
- \`round-limit\`（必填，除非批次已提供）：子代理最多生成几轮。
- \`time-limit\`（必填，除非批次已提供）：子代理最多运行多久，如 \`90s\`、\`5m\`、\`1h\`。
- \`ai-source\`（可选）：指定 AI 源名；缺省继承你的 AI 源，再缺省用默认 AI 源。
- \`async="true"\`（可选）：异步运行，立即返回 backgroundId；完成后会以系统消息通知你。
...
export function getSubAgentPrompt(args) {
	...
	if (parentRunId) {
		...
		const notifications = takePendingNotifications({ username: args.username, charId: args.char_id, parentRunId: null })
		additionalChatLog.push(...notifications)
	}

	if (parentRunId) {
		const notifications = takePendingNotifications({
