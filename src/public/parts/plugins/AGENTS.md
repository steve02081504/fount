...
- **Unified background tasks**: register long-running background work with `plugins/async-task/registry.mjs` rather than rolling a private promise registry. Settled results remain awaitable during the spawning generation (`ownerFromArgs` carries `generationId`); the parent runner calls `finishAsyncGeneration(id)` after its regen loop to release unclaimed results. Notifications still go out for tasks not awaited while running; `<list-async/>` lists only running tasks. Expose `<list-async/>` / `<await-async>` through your `GetPrompt` + `ReplyHandler`, or rely on the `async-task` plugin's own.
- **JS eval cwd**: `<run-js>` / `<inline-js>` evaluate in-process via `async_eval`, so `process.cwd()` is the fount process cwd and relative `fs` paths do not follow the shell/workdir target; never `process.chdir` (process-wide, races across concurrent generations). The local JS context injects `workdir` (target workdir absolute path, omitted for remote execution) so the model can build absolute paths itself — see `code-execution/handler.mjs` `getJsEvalContext`.
