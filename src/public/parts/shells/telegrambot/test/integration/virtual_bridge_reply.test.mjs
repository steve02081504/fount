...
Deno.test('telegram virtual bridge: message → GetReply → sendMessage', async () => {
	...
	assert(showSent[0].text.includes('前面后面'), `outbound should use content_for_show: ${showSent[0].text}`)
	assert(!showSent[0].text.includes('gentian-sticker'), 'reply-handler trigger must not leak to platform')
})
