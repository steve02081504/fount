...
async function loadUserGroups() {
	try {
		...
		container.innerHTML = groups.map(group => `
			<a href="/parts/shells:chat/group.html#${group.groupId}" class="flex items-center gap-3 bg-base-300/50 rounded-xl px-4 py-3 hover:bg-base-300 transition-colors cursor-pointer">
...
async function loadUserChannels() {
	try {
		...
		for (const group of groups) {
			try {
				const stateRes = await fetch(`/api/parts/shells:chat/${group.groupId}/state`, {
					credentials: 'include'
				})
		...
		container.innerHTML = allChannels.map(ch => {
			...
			return `
			<a href="/parts/shells:chat/group.html#${ch.groupId}" class="flex items-center gap-3 bg-base-300/50 rounded-xl px-4 py-3 hover:bg-base-300 transition-colors cursor-pointer">
