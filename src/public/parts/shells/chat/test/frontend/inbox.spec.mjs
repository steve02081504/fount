...
test.describe('Inbox', () => {
	...
	test('seeded @mention shows badge, inbox list, and jump to message', async ({ page, baseUrl, apiKey, groupChannel }) => {
		...
		await expect(mentionTab).toHaveClass(/tab-active/)
		await mentionTab.focus()
		await page.keyboard.press('ArrowRight')
		await expect(messageTab).toHaveAttribute('aria-selected', 'true')
		await expect(messageTab).toHaveClass(/tab-active/)
		...
		await expect(messageInput).toHaveJSProperty('disabled', false, { timeout: ms('2m') })
		await expect(messageInput).toHaveJSProperty('value', '')
