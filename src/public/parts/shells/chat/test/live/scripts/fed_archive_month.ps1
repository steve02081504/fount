...
$setup = Setup-OpenGroupJoin 'FedArchive' 'archive-seed-0'
...
T 'A set hotLatestMessageCount=1' {
	$r = Api $FedA PUT "/groups/$gid/settings" @{ hotLatestMessageCount = 1 }
	$r.status -eq 200
}
T 'A posts archive candidates' {
	for ($i = 1; $i -le 4; $i++) {
...
T 'B POST federation/offline-mark' {
...
T 'B can read archived message via GET messages' {
