...
await testCase('A POST /groups/:id/emoji-packs/:packId/emojis multipart', async () => {
	const r = await ApiMultipart(FedA, 'POST', `/groups/${gid}/emoji-packs/${gid}/emojis`, { name: 'fed-emoji' }, 'emoji', 'fed.png', FedPngBytes)
...
await testCase('A GET emoji data locally', async () => {
	const r = await Api(FedA, 'GET', `/groups/${gid}/emoji-packs/${gid}/emojis/${emojiId}/data?json=1`)
...
await testCase('B sees emoji in manifest', async () => pollUntil(async () => {
	...
	return r.status === 200 && (r.json.pack?.items?.filter(e => e.emojiId === emojiId).length ?? 0) >= 1
}, 60, 3))

await testCase('B GET emoji-packs/:packId/emojis/:id/data (federation fetch)', async () => {
	const ok = await pollUntil(async () => {
		const r = await Api(FedB, 'GET', `/groups/${gid}/emoji-packs/${gid}/emojis/${emojiId}/data?json=1`)
...
await testCase('B data matches A (same length)', async () => {
	const a = await Api(FedA, 'GET', `/groups/${gid}/emoji-packs/${gid}/emojis/${emojiId}/data?json=1`)
	const b = await Api(FedB, 'GET', `/groups/${gid}/emoji-packs/${gid}/emojis/${emojiId}/data?json=1`)
