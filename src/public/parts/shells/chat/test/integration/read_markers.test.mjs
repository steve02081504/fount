...
Deno.test('message seq monotonic and read-marker clears unread summary', async () => {
	...
	const beforeMarker = getChannelReadMarker(username, operator, groupId, channelId)
	...
	setChannelReadMarker(username, operator, groupId, channelId, { eventId: first.eventId, seq: first.seq })

	const marker = getChannelReadMarker(username, operator, groupId, channelId)
	...
	const groups = await enumerateJoinedFederatedGroups(username, operator)
