...
Deno.test('synthesize reads reactions from overlay after fold (events.jsonl gone)', () => {
	...
	assertEquals(rows.every(r => r.type === 'reaction_add' && r.content.targetId === messageEventId && r.content.emoji === '👍'), true)
...
Deno.test('synthesize handles multi-codepoint emoji containing colons', () => {
	...
	assertEquals(rows[0].content.targetId, messageEventId)
})
