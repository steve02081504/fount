...
/**
 * 构造最小判权物化状态：根容器 + 治理默认频道 + 普通频道，子频道默认继承根权限块。
...
Deno.test('canInChannel: group permission evaluates against group scope, not caller channelId', () => {
	...
	// secret 跟随根块：在根块上 deny 掉 MANAGE_ROLES（频道层面 deny，但治理权限按群 scope 求值不受影响）。
	state.channelPermissions.root = {
		moderator: { allow: {}, deny: { MANAGE_ROLES: true } },
	}
	assertEquals(governanceChannelId(state), 'default')
	// 治理权限按群 scope 求值：即使频道 root 覆写 deny MANAGE_ROLES，仍返回 true。
	assertEquals(canInChannel(state, state.members[MOD], PERMISSIONS.MANAGE_ROLES, 'secret'), true)
})
...
Deno.test('canInChannel: channel-level permission honors the passed channelId override', () => {
	...
	assertEquals(canInChannel(state, state.members[MOD], PERMISSIONS.SEND_MESSAGES, 'secret'), false)
})
...
Deno.test('canInChannel: array of permissions is satisfied when any granted (mixed scope)', () => {
	...
	// 数组中仅一个满足即 true。
	assertEquals(canInChannel(state, state.members[MOD], [PERMISSIONS.MANAGE_ROLES, PERMISSIONS.KICK_MEMBERS], 'secret'), true)
	// 全部不满足 → false。
	assertEquals(canInChannel(state, state.members[MOD], [PERMISSIONS.KICK_MEMBERS, PERMISSIONS.BAN_MEMBERS], 'secret'), false)
})
