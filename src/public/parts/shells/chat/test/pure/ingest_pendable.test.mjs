...
Deno.test('dag_tip_merge no fork is pendable', async () => {
	await ensureServer()
	const groupId = 'g-merge-no-fork'
	...
	assertEquals(thrown?.pendable, true)
	assertEquals(thrown?.message, 'dag_tip_merge: no fork')
})

Deno.test('dag_tip_merge prev_event_ids mismatch is pendable', async () => {
	await ensureServer()
	const groupId = 'g-merge-mismatch'
	const path = eventsPath(username, groupId)
	await mkdir(dirname(path), { recursive: true })
	const rowA = { id: TIP_A, type: 'message', prev_event_ids: [], sender: SENDER }
	const rowB = { id: TIP_B, type: 'message', prev_event_ids: [], sender: SENDER }
	await writeFile(path, `${JSON.stringify(rowA)}\n${JSON.stringify(rowB)}\n`, 'utf8')
	...
	assertEquals(thrown?.pendable, true)
	assertEquals(thrown?.message, 'dag_tip_merge: prev_event_ids must list all current DAG tips')
})
