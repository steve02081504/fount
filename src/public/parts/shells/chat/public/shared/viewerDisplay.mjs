...
/**
 * 角色私聊侧栏中 viewer 的展示名：优先异步就绪的 `viewerDisplayName`；
 * 未就绪时回退到当前群 viewer 成员的 displayName（profile 名），最后落到 entityHash 短码。
...
export function resolveViewerSidebarDisplayName({ viewerDisplayName, entityHash, memberDisplayName, alias = '' } = {}) {
	if (viewerDisplayName) return viewerDisplayName
	return resolveDisplayName({
		entityHash,
		alias,
		profileName: memberDisplayName || undefined,
	})
}
