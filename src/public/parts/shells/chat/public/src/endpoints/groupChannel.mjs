/**
 * 【文件】public/src/endpoints/groupChannel.mjs
...
export async function searchAllChatGroups(query, options = {}) {
	...
	const data = await chatFetch(`/search?${params}`)
	return {
 ...
 * @returns {Promise<void>} 无
 */
export async function putReaction(groupId, channelId, targetEventId, emoji) {
	await groupFetch(groupPath(groupId, 'channels', channelId, 'reactions'), {
		method: 'POST',
		json: { targetEventId, emoji },
	})
}

/**
 * 撤销一条表情回应（管理员可代撤 `targetPubKeyHash`）。
 * @param {string} groupId 群 ID
 * @param {string} channelId 频道 ID
 * @param {string} targetEventId 目标消息事件 ID
 * @param {string} emoji 表情字符
 ...
 * @returns {Promise<void>} 无
 */
export async function deleteReaction(groupId, channelId, targetEventId, emoji, targetPubKeyHash) {
	await groupFetch(groupPath(groupId, 'channels', channelId, 'reactions'), {
		method: 'DELETE',
		json: {
			targetEventId,
 ...
 * @param {string} groupId 群 ID
 * @param {string} channelId 频道 ID
 * @returns {Promise<{ markers: Record<string, { seq: number, eventId: string }> }>} 成员已读水位映射
 */
export async function getMemberReadMarkers(groupId, channelId) {
	return groupFetch(groupPath(groupId, 'channels', channelId, 'member-read-markers'), { method: 'GET' })
}
 ...
 * @param {string} groupId 群 ID
 * @param {string} channelId 频道 ID
 * @returns {Promise<{ active: boolean, peerCount?: number }>} 通话状态
 */
export async function getCallStatus(groupId, channelId) {
	return groupFetch(groupPath(groupId, 'channels', channelId, 'call-status'), { method: 'GET' })
}
