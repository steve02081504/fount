...
async function renderGroupEmojis(ctx) {
	const container = document.getElementById('group-emojis-container')
	if (!container || !ctx.groupId) return
	const channelId = ctx.state?.groupSettings?.defaultChannelId || 'default'
	const [canManage, entries] = await Promise.all([
		viewerCanManageMessages(ctx.state, ctx.groupId, channelId).catch(() => false),
		fetch(`/api/parts/shells:chat/groups/${encodeURIComponent(ctx.groupId)}/emojis`, { credentials: 'include' })
			.then(r => r.ok ? r.json() : {})
			.then(d => Array.isArray(d.entries) ? d.entries : [])
			.catch(() => []),
	])
	const entriesHtml = entries.map(entry => {
		const src = groupEmojiDataApiPath(ctx.groupId, entry.emojiId)
		const del = canManage
...
${del}
	...
	if (upload)
		upload.addEventListener('change', async () => {
			...
			const up = await fetch(`/api/parts/shells:chat/groups/${encodeURIComponent(ctx.groupId)}/emojis`, {
				method: 'POST',
				credentials: 'include',
				body: form,
			})
			...
			await ensureGroupEmojisPanel(ctx)
		})
	...
	container.querySelectorAll('[data-delete-emoji]').forEach(deleteEmojiButton => {
		deleteEmojiButton.addEventListener('click', async () => {
			...
			const del = await fetch(
				`/api/parts/shells:chat/groups/${encodeURIComponent(ctx.groupId)}/emojis/${encodeURIComponent(emojiId)}`,
...
/** @param {import('./state.mjs').GroupSettingsContext} ctx @returns {Promise<void>} */
export async function ensureGroupEmojisPanel(ctx) {
	if (!ctx.groupId || ctx.emojisPanelReady) return
	ctx.emojisPanelReady = true
	await renderGroupEmojis(ctx)
}
