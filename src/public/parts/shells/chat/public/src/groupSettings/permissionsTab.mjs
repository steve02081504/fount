...
export async function renderPermissionSettings(ctx) {
	const container = document.getElementById('permission-settings-container')
	if (!container) return
	if (!ctx.settingsCaps?.canManageRoles) {
	...
	ctx.permissionsController?.abort()
	ctx.permissionsController = new AbortController()
	const { signal } = ctx.permissionsController
	...
	document.getElementById('group-settings-create-role-button').addEventListener('click', () => {
		showCreateRoleModal(ctx)
	}, { signal })
	container.addEventListener('change', async event => {
		const checkbox = event.target.closest('[data-action="update-permission"]')
		if (checkbox) await updateRolePermission(ctx, checkbox.dataset.roleId, checkbox.dataset.perm, checkbox.checked)
	}, { signal })
	container.addEventListener('click', async (clickEvent) => {
		...
		if (deleteRoleButton) await deleteRole(ctx, deleteRoleButton.dataset.roleId)
	}, { signal })
}
 ...
 */
async function updateRolePermission(ctx, roleId, permission, enabled) {
	const resp = await fetch(`/api/parts/shells:chat/groups/${encodeURIComponent(ctx.groupId)}/roles/${encodeURIComponent(roleId)}/permissions`, {
		method: 'PUT',
...
async function deleteRole(ctx, roleId) {
	...
	const resp = await fetch(`/api/parts/shells:chat/groups/${encodeURIComponent(ctx.groupId)}/roles/${encodeURIComponent(roleId)}`, {
		method: 'DELETE',
		credentials: 'include'
	})
...
function showCreateRoleModal(ctx) {
	const name = promptI18n('chat.group.settingsPage.createRolePrompt')
	if (!name?.trim()) return

	fetch(`/api/parts/shells:chat/groups/${encodeURIComponent(ctx.groupId)}/roles`, {
		method: 'POST',
		headers: { 'Content-Type': 'application/json' },
		credentials: 'include',
