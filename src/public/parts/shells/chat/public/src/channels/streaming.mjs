/**
 * 【文件】public/src/channels/streaming.mjs
 * 【职责】流媒体频道 WebRTC：多方网状拓扑、经群 WS 交换 webrtc_signal。
 * 【原理】startAvSession 建立 RTCPeerConnection；信令 { type, channelId, from, to?, signal } 经 groupWsClient 广播；TURN 配置来自 getStreamingChannelAuth。
 * 【数据结构】参与者 peerId 映射、MediaStream、信令 payload。
...
/**
 * @param {{ iceServers?: RTCIceServer[] }} [auth] streaming-auth 响应
 * @returns {RTCIceServer[]} ICE 服务器列表
 */
function getIceServers(auth) {
	if (Array.isArray(auth?.iceServers) && auth.iceServers.length)
		return auth.iceServers
 ...
 */
async function getOrCreatePeerTile(peerId, avGrid) {
...
function updateGridLayout(avGrid) {
	if (!avGrid) return
	const count = avGrid.querySelectorAll('.streaming-av-tile').length
	let cols = count <= 1 ? 1 : count <= 4 ? 2 : 3
	if (avGrid.querySelector('.streaming-av-tile-main'))
		cols = 1
...
function toggleMainView(tile, avGrid) {
	const isMain = tile.classList.contains('streaming-av-tile-main')
	avGrid.querySelectorAll('.streaming-av-tile-main').forEach(t => {
		t.classList.remove('streaming-av-tile-main', 'col-span-full', 'row-span-2')
	})
	if (!isMain)
		tile.classList.add('streaming-av-tile-main', 'col-span-full', 'row-span-2')
 ...
 * @property {(signalMessage: object) => void} handleSignal  处理收到的 WS webrtc_signal 消息
 ...
 * @param {{ iceServers?: RTCIceServer[] }} [p.iceAuth] 群 ICE 配置（来自 streaming-auth）
 * @returns {Promise<AvSession>} 音视频会话控制对象
 */
export async function startAvSession(p) {
	const { channelId, clientId, videoLocal, remoteContainer, broadcast, iceAuth } = p
	const iceServers = getIceServers(iceAuth)
	...
	function createPc(peerId) {
		...
		pc.ontrack = ev => {
			...
			void getOrCreatePeerTile(peerId, remoteContainer).then(tileInfo => {
				if (!tileInfo) return
	 ...
	 * @param {object} signalMessage 信令消息体
	 * @returns {Promise<void>}
	 */
	...
	return {
		...
		close: async () => {
			...
			remoteContainer?.querySelectorAll('.streaming-av-tile-main').forEach(t => {
				t.classList.remove('streaming-av-tile-main', 'col-span-full', 'row-span-2')
			})
			if (remoteContainer) updateGridLayout(remoteContainer)
