...
/**
 * 将消息内容中的 [sticker:id|url] 和 [sticker:id] 标签转换为 HTML img 元素。
 * @param {string} content - 原始消息内容
 * @returns {string} 处理后的内容
 */
function processStickerTags(content) {
	if (typeof content !== 'string') return content || ''
	return content.replace(/\[sticker:([^\]|]+)\|([^\]]+)\]/g, (_, id, url) => {
		const safeUrl = url.replace(/"/g, '&quot;')
		return `<img src="${safeUrl}" alt="sticker" class="inline-block max-w-[120px] max-h-[120px] rounded-lg" data-sticker-id="${id}" />`
	}).replace(/\[sticker:([^\]]+)\]/g, (_, id) => {
		return `<span class="inline-block px-2 py-1 bg-base-300 rounded text-sm opacity-70" data-sticker-id="${id}">🖼️ 贴纸</span>`
	}).replace(/\[image:([^\]|]+)\|([^\]]+)\]/g, (_, name, url) => {
		const safeUrl = url.replace(/"/g, '&quot;')
		return `<img src="${safeUrl}" alt="${name}" class="block max-w-[300px] max-h-[300px] rounded-lg my-1 cursor-pointer" onclick="window.open(this.src,'_blank')" />`
	})
}
...
export async function renderMessage(message) {
	...
	let rawContent = message.content_for_show || message.content
	// 预处理贴纸标签：[sticker:id|url] -> <img>，[sticker:id] -> 占位符
	rawContent = processStickerTags(rawContent)

	const preprocessedMessage = {
		...message,
		avatar: message.avatar || DEFAULT_AVATAR,
		time_stamp: new Date(message.time_stamp).toLocaleString(),
		content: await renderMarkdownAsString(rawContent, cache),
	}
