...
- **Message.content (ChatClient / chat_log / OnMessage) is fount text (`string`)**. DAG wire is separate: discriminated `type` (`text` omits `type` to save memory; `sticker`/`vote`/`group_invite`/`call` keep top-level fields). Convert at hydrate (`chatLogEntry_t`) / serialize boundaries — chars must not unpack wire themselves. Shell sidecars live only under `extension.chat` (`eventId`, `channelId`, `bridge`, `replyTo`, … — not kind payloads). `fileId` exists only on wire `files[]`; hydrated / ChatClient `files[].buffer` is a lazy getter. Wire types: `decl/channelWire.ts`.
