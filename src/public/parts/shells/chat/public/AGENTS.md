...
- **`member_join` binding**: `bindingSig` + `verifyEntityActivePubKeyBelongs` (local identity or EVFS signed `profile.json`) — cannot spoof another entityHash with a self-made active key.
