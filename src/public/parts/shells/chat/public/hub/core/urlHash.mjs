 ...
 * 格式：`#group:{groupId}`、`#group:{groupId}:{channelId}` 或 `#group:{groupId}:{channelId};{eventId}`。
 * 频道段可缺省（群无默认频道/尚未选择频道时），此时 `channelId` 为 null。
...
export function parseHash() {
	...
	try {
		const rawGroup = decodeURIComponent(sep < 0 ? rest : rest.slice(0, sep))
		const channelPart = sep < 0 ? '' : rest.slice(sep + 1)
		// `;{eventId}` 后缀
		...
		if (!groupId) return { groupId: null, channelId: null, eventId: null }
		return { groupId, channelId: channelId || null, eventId: eventId || null }
	}
	catch {
 ...
 * 格式：`#group:{groupId}`（无频道）、`#group:{groupId}:{channelId}` 或 `#group:{groupId}:{channelId};{eventId}`
...
export function updateHash(groupId, channelId, eventId) {
	if (!groupId) return
	let newHash = `group:${encodeURIComponent(groupId)}`
	if (channelId) newHash += `:${encodeURIComponent(channelId)}`
