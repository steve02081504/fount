...
- Prefer `renderTemplate` / `mountTemplate` / `openDialogFromTemplate`; cross-shell shared modules use `withTemplates`. DaisyUI + shared `promptDialog` / `positionContextMenu`. From `public/hub/*.mjs` import `public/src` as `../src/...` (`../../src` → `/parts/src` 404).
