...
- **Entity profile card**: Hub 点击头像/作者 → `profilePopup.mjs`；跨壳轻量卡 → `/parts/shells:chat/shared/entityProfilePopup.mjs`。所属方框 / 归因警告 → `shared/entityProfileCard.mjs`（`--color-warning`）。导入历史等 attribution mismatch ≠ Ed25519 验签失败。
