...
- **Untrusted remote Markdown**: Hub `messages/render/markdown.mjs` uses `renderMarkdownAsString(..., { allowDangerousHtml })`; local messages and own/local-agent content (`isTrustedMarkdownAuthor` + `nodeHash` / `ownerEntityHash`) take the trusted tier, all others the safe tier. First frame renders immediately; untrusted oversized text shows a preview + expand button.
...
- **Profile bio Markdown**: `paintEntityProfileBio` → same two tiers; self / own agent / trust list. Reused by Cabinet / Social.
