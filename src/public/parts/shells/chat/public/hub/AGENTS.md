...
- **`?contact=` / remote profile**: Hub `GET …/entities/:hash` uses `fetchRemote`; profile path first assembles from local cached public manifest/chunks, then races network `readPublicFile` with `REMOTE_PROFILE_FETCH_TIMEOUT_MS` (must cover EVFS manifest+chunk fanout) + negative cache (`profile.mjs`). Until [fount-p2p#8](https://github.com/steve02081504/fount-p2p/issues/8) lands, local-first avoids false misses when cache is already warm. Popup paints a stub name/avatar before the fetch returns so a hung peer cannot leave an empty card. DM button shows for any non-self `entityHash`; click (and `dispatchFriendChat`) resolve `activePubKeyHex` via `forceRemote=1` when the contact link only carried the hash.
