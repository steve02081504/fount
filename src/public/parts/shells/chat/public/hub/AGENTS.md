...
- **Main text channel read**: `GET …/view-log` via `getChannelViewLog` (viewer-filtered row DTOs; same shape as raw `/messages`). Raw `GET …/messages` is only for moderation/debugging/navigation backfill.
