...
- **Untrusted remote Markdown**: `messages/render/markdown.mjs` (`hydrateMessageMarkdown`) renders the first 120 chars as preview via the untrusted pipeline (aligned with mention `textPreview`); overflow shows an expand button; trusted authors still use the trusted pipeline (`allowDangerousHtml`). Pending source is `registerPendingMessageMarkdown` (in-memory Map) + `data-md-pending` — **never** put raw markdown into `data-md-raw` attributes (HTML quotes break attribute parsing → blank bubbles).
