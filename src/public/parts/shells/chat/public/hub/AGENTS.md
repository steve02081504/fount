...
- **No default channel**: `groupSettings.defaultChannelId` may be `null` (DM always; normal groups after deleting their default). Landing order is `hash/preset → defaultChannelId → topmost openable channel` (`sidebar/firstOpenableChannel.mjs`, pre-order over root `links`, skips category/thread); if none, `sidebar/noChannelState.mjs` mounts the `hub/empty/no_channel` empty state with a create-channel CTA. `#group:{gid}` (no channel segment) is a valid hash — `parseHash` returns `channelId: null`, `updateHash` omits the segment. Never fabricate `'default'`. The channel context menu shows set-default (`setDefault`) or unset-default (`unsetDefault`) depending on whether the channel is the current default.
...
- **Message multi-select**: `hub/messages/messageSelection.mjs` binds the shared interval `selectionController` to `#messages` (ordered ids from `store.messages.channelMessages`, not DOM). A native text selection spanning different `.message-row`s auto-upgrades to message selection (`bindSelectionUpgrade`); selected rows get `.is-selected` (re-applied on virtual-list rerender via `syncMessageSelectionStyles`), Ctrl/⌘+C copies `HH:MM · sender` + body as HTML+text, clicking blank space / Esc / search-input / channel switch clears. `messageDragExport` no-ops while selection is active. Selection survives virtual queue (O(segments) memory), so keep `getOrderedIds` returning a stable array.
