...
- **Profile bio Markdown**: `paintEntityProfileBio` → `shared/trustedMarkdown.mjs`（Social `mountMarkdown` / `renderTrustedPostMarkdown` 同入口）；self / own agent / declared master / trust list. Reused by Cabinet / Social.
