...
function localizedIsBlank(localized) {
	...
	return keys.every((key) => {
		...
		return !(slice.name || '')
			...
			&& !(slice.sfw_avatar || '')
			&& !(slice.description || '')
			&& !(slice.sfw_description || '')
			&& !(slice.description_markdown || '')
			&& !(slice.sfw_description_markdown || '')
...
async function materializePartAvatar(username, charname, avatarRaw) {
	...
	if (raw.startsWith('data:')) {
		...
		return { buffer, mimeType, filename: 'avatar' }
	}
	...
	if (raw.startsWith('http://') || raw.startsWith('https://')) {
		...
		const ext = Object.entries(IMAGE_MIME_BY_EXT).find(([, mime]) => mime === mimeType)?.[0]
			...
			|| 'png'
		return { buffer, mimeType, filename: `avatar.${ext}` }
	}
