 ...
 * 【原理】跳过问候条目与无 eventId 的条目；resolveLocalEventSigner 得 sender；content 含 targetId、feedbackType、`extension.chat.entryId`；失败仅 console.error 不抛。
...
export async function mirrorFeedbackToDag(groupId, entry, feedback, username) {
	try {
		...
		const channelIdForDag = await resolveGroupChannelId(username, groupId, entry.extension?.chat?.channelId)
