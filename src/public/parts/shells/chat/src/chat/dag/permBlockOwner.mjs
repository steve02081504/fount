/**
 * 【文件】dag/permBlockOwner.mjs
 * 【职责】沿 `permBlockId` 指针解析频道权限块来源（循环保护），供 reducer 与物化状态复用。
 */
...
export function resolvePermBlockOwner(state, channelId) {
	...
	while (current && !seen.has(current)) {
		...
		if (!channel) break
		const permissionBlockId = channel.permBlockId
		if (!permissionBlockId) break
		current = permissionBlockId
	}
	return current
}
