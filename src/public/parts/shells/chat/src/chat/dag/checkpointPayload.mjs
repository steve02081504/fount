...
export function buildCheckpointPayload({
	...
	const members_record = {
		...
		categories: JSON.parse(JSON.stringify(materialized.categories || {})),
		categoryPermissions: JSON.parse(JSON.stringify(materialized.categoryPermissions || {})),
	...
	if (batterySaver) {
		const aclSlice = {
			...
			categories: members_record.categories,
			categoryPermissions: members_record.categoryPermissions,
