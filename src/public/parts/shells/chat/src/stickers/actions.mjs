/**
 * 【文件】stickers/actions.mjs
 * 【职责】CLI/IPC 贴纸命令表：列出、创建、安装/卸载包等无 HTTP 入口。
 * 【原理】各 action 以 replica 用户名为磁盘所有者，直接调用 stickers.mjs 领域函数并返回人类可读字符串。
 * 【数据结构】actions 对象、packId/name/description 参数、贴纸包摘要行文本。
 * 【关联】被 chat shell handleAction 引用；依赖 stickers.mjs。
 */
import {
	getStickerPacks,
	createStickerPack,
