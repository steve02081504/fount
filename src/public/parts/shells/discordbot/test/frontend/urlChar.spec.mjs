/**
 * Discord bot URL `char` 参数模板预填回归。
 * 回归对象：`?name=X&char=Y` 下新建 bot 应预填角色配置模板，而非留 `{}`。
 */
import { test, expect } from './fixtures.mjs'

test.describe('Discord bot URL char template prefill', () => {
	test('URL char param pre-fills a fresh bot with the char config template', async ({ page, baseUrl }) => {
		...
		await page.goto(`${baseUrl}/parts/shells:discordbot/?name=${botname}&char=urlChar`, {
	...
	test('URL char param does not clobber a saved non-empty config', async ({ page, baseUrl }) => {
		...
		const res = await page.request.post(`${baseUrl}/api/parts/shells:discordbot/setbotconfig`, {
			data: { botname, config: { token: 'test-token', char: 'saved-char', config: { OwnerUserName: 'SAVED_OWNER' } } },
		})
