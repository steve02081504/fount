...
async function findAndConnect() {
	...
	for (const { host, protocol } of uniqueHosts) try {
		...
		try {
			const response = await gmFetch(`${protocol}//${host}/parts/shells:browserIntegration/script.user.js`, {
				headers: {
			...
			if (remoteVersion && remoteVersion !== GM_info.script.version)
				if (window.confirm(await geti18n('browser_integration_script.update.prompt')))
					window.open(`${protocol}//${host}/virtual_files/parts/shells:browserIntegration/script.user.js`, '_blank')
		} catch (error) {
...
function connect(host, protocol, username, apikey) {
	...
	return new Promise((resolve, reject) => {
		...
		ws = new WebSocket(`${wsProtocol}//${host}/ws/parts/shells:browserIntegration/page`, apikey)
		/**
			...
			case 'run_js': {
				...
				if (callbackInfo)
					...
					callback = async (data) => {
						const { host, protocol } = await getStoredData()
						if (!host) return
						try { await makeApiRequest(host, protocol, '/api/parts/shells:browserIntegration/callback', { method: 'POST', data: { ...callbackInfo, data, pageId, script } }) }
