...
- Record shape: `{ id, parentId?, charId, charname?, subAgent?, chatId?, conversationId?, source, startedAt, finishedAt?, input?, requests?, requestCount?, dialogue?, response?, conversation?, model?, metadata?, error? }`. `input` is a legacy request `chat_log` snapshot (not the assembled prompt). `requests` is the per-round AI request snapshot: `{ index, startedAt, finishedAt, model, systemPrompt, messages, snapshot? }` (`systemPrompt` = merged system prompt, `messages` = visible chat log after `mergeStructPromptChatLog`, each message carries an `id` and a `type` when the source entry had one — `cache-report` uses `type: 'summary'` to exclude compression rounds; `snapshot` = the AI source's `BuildPrompt` output deep-serialized to plain text, binary hashed — see below). `dialogue` is the reconstructed conversation `{ rounds, events }` derived from `requests` + the final reply.
- **Active recording** (`src/request_record.mjs`, imported by the caller, never injected via `generation_options`): `beginPromptRequest(args, prompt_struct, { model, aiSource })` (async) before each `StructCall`, passing the AI source so the snapshot uses its `BuildPrompt`; `finishPromptRequest(handle, { error })` after, and `finishGeneration(args, { response, error, metadata })` at generation end. Callers: the char templates (ZL-31/ln, easychar, SillyTavern, Risu) and `plugins/sub-agent` runtime; benchmark records come from the char template the runner calls. A request the caller never records produces no history — never fall back to `input` as if it were the full prompt.
...
- **Reconstruction** (`public/shared/dialogueReplay.mjs`): messages align by `id` across rounds; new char/tool messages first visible in request *N+1* are outputs of round *N* (not future prompt inputs); user/system inputs and edits stay at their request round. The final reply belongs to the last round. `getGeneration` rebuilds the dialogue from retained requests at read time so existing records with prompt snapshots also display the corrected chronology; when requests expire, the retained dialogue is used.
...
- Conversation grouping: `conversationKey = conversationId || chatId || id`; `summarizeConversations` (both in `public/shared/generationChain.mjs`) powers `listConversations` / `getConversation`. `getConversation` merges per-generation `dialogue` events (rounds offset in start order) into one continuous `dialogue` (`{ rounds, events, messages }`); a generation whose final reply was superseded (the next generation replayed the same user turn without carrying that reply) has that reply dropped from the merged transcript, so a re-run/continuation does not stack parallel answers.
...
- Run's `conversation` is display history, while `promptConversation` supplies `childArgs.chat_log`: handler logs already present in `additional_chat_log` go to display history only, or they would be doubled in the next prompt. Live `AddChatLogEntry` writes both; `injectRoundEntries` picks up new user entries at the next model round. `POST /subagent/:runId/messages` is user-scoped and rejects after the active run's in-memory context disappears; do not extend run lifetime just to support sending.
...
- Prompt cache UI is an **estimate** based on the character length of contiguous common prefixes across prompt snapshot plain text, not provider-reported billed cached tokens. The text compared is the record's `snapshot`: the AI source's `BuildPrompt(prompt_struct)` output (its outbound-form JSON structure; attachments stay `Buffer`) recursively serialized to tab-indented `\n` text with binary reduced to a variable-length hash (50KB → 2 bytes per chunk) by `chat/src/prompt_struct/serializeSnapshot.mjs`; records without a `snapshot` (old data / sources lacking `BuildPrompt`) fall back to `systemPrompt` + `messages`. `public/shared/promptCache.mjs` is the pure source of this metric (`estimatePromptCache` for a conversation view, `estimateGenerationCache` for one generation against a supplied previous prompt). A generation's summary `rate` is the **arithmetic mean of its per-round rates** (each round normalized by its own prompt length), not `reused / sum(round lengths)` — the latter halves a genuine near-100% prefix hit.
...
`/api/parts/shells:agent_studio/` — `/chars`, `/char/:id/overview`, `/subagents?charId=|chatId=`, `/subagent/:runId` (GET status; POST `/:runId/messages`), `/generations` (GET list / DELETE clear, filter `charId` etc., no filter wipes all), `/generation/:id`, `/conversations`, `/conversation/:key`, `/chains`, `/retention`, `/benchmarks` CRUD, `/benchmarks/:id/run`, `/runs`. Sub-agent views derive from generation records grouped by `subAgent.runId` / `batchId`; live state is imported from `plugins/sub-agent/state.mjs`.
...
- Four main views, each `<section id="<view>View" class="view">` in `public/index.html`: `dashboard` (char list + overview, sub-agent run cards and conversation list), `generations` (conversations + chains, char filter), `benchmarks` (defs + runner + JSON import/export + results), `settings` (retention). `#conversation/<key>` is the only detail view, including `#conversation/subagent%3A<runId>`; its replay slider / timeline is **per round** (`buildRoundUnits` expands each generation into one unit per `requestCount` / `dialogue.rounds` — the span formula must stay identical to `mergeDialogueEvents`, else replay misaligns; generation boundaries are the `generation-start` nodes). Each revealed round inside its generation card shows the result of that call, its own cache badge, and its own collapsible prompt; the final response and completion state appear in the last round only. The cache-rate chart is **per round** (one point per timeline node, each round's own request-vs-previous rate from `estimatePromptCache`'s `rounds`). `messageBody.mjs` gives every message a Markdown/plain-text switch, defaulting to plain text. Every prompt text block (system prompt / messages / response / sub-agent task / generation-dialog prompt & response) carries copy + download `.section-actions` (`lib/textActions.mjs`; `shared/promptText.mjs` serializes message arrays) for the prompt-improvement flow. The sub-agent transcript is fetched from its live run while active, from persisted history afterward; its composer is shown only at the latest replay node while `canSend` is true. `src/viewChrome.mjs` toggles visibility/highlight; `src/navigation.mjs` maps view → loader, syncs `location.hash`, and wraps switches in a View Transition.
