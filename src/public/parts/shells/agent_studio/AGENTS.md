...
- Record shape: `{ id, parentId?, charId, charname?, subAgent?, chatId?, conversationId?, source, startedAt, finishedAt?, input?, requests?, requestCount?, dialogue?, response?, conversation?, model?, metadata?, error? }`. `input` is a legacy request `chat_log` snapshot (not the assembled prompt). `requests` is the per-round AI request snapshot: `{ index, startedAt, finishedAt, model, systemPrompt, messages }` (`systemPrompt` = merged system prompt, `messages` = visible chat log after `mergeStructPromptChatLog`, each message carries an `id` and a `type` when the source entry had one — `cache-report` uses `type: 'summary'` to exclude compression rounds). `dialogue` is the reconstructed conversation `{ rounds, events }` derived from `requests` + the final reply.
