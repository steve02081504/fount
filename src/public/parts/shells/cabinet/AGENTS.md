...
- **Frontend layout**: `public/index.mjs` is bootstrap only; page state in `public/src/state.mjs` (`cabinetStore`); domain modules under `public/src/` (`navigation`, `remoteBrowse`, `entryGrid`, `entryActions`, `contextMenu`, `commands`, `properties`, `wiring`). `keyboard.mjs` stays pure shortcut matching.
