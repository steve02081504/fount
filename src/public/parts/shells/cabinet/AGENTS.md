...
- **Tests**: `fount test shells/cabinet --no-parallel`. Pure suites import `public/shared/` only. Keep Social `visibilitySpec` behind dynamic import in `remote.mjs` so pure suites do not statically pull Social.
