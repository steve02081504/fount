...
- **Tests**: `fount test shells/cabinet --no-parallel`. Pure suites that import `remote.mjs` sanitize helpers must not statically pull Social (`visibilitySpec`) — keep that behind dynamic import in fetch paths.
