...
export async function deleteSelection() {
	...
	if (rows.some(row => row.attrs?.system) && !await confirmAction('cabinet.confirmDeleteSystem')) return
	if (!rows.some(row => row.attrs?.system) && !await confirmAction('cabinet.confirmDelete')) return
