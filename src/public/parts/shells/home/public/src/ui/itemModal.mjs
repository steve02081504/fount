...
/**
 * 创建一个给定部件的动作按钮。
 * @param {object} part - 包含部件类型、部件名称、部件详细信息和部件路径的部件对象。
 * @param {Array<object>} interfacesRegistry - 接口配置数组。
 * @returns {HTMLAnchorElement[]} 一个按钮元素数组。
 */
export function createActionButtons(part, interfacesRegistry) {
	const { partpath, partdetails } = part
	const normalizedPartpath = partpath.replace(/^\/+|\/+$/g, '')
	const [parttype, ...rest] = normalizedPartpath.split('/')
...
export async function showItemModal(part) {
	const { partpath, partdetails } = part
	const normalizedPartpath = partpath.replace(/^\/+|\/+$/g, '')
	const [parttype] = normalizedPartpath.split('/')
	const dataForTemplate = { ...partdetails, defaultIcon: defaultIcons[parttype] || genericDefaultIcon }
	...
	window.VanillaTilt?.init?.(modalView.querySelector('.modal-image-wrapper'), {
		max: 15,
		speed: 400,
		glare: true,
		'max-glare': 0.5,
