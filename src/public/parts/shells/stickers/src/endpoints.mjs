...
export function setEndpoints(router) {
	router.get('/api/parts/shells:stickers/packs', authenticate, async (req, res) => {
		try {
	...
	router.post('/api/parts/shells:stickers/packs', authenticate, async (req, res) => {
		try {
			const { username } = await getUserByReq(req)
	...
	router.put(/^\/api\/parts\/shells:stickers\/packs\/([^/]+)$/, authenticate, async (req, res) => {
		try {
	...
	router.delete(/^\/api\/parts\/shells:stickers\/packs\/([^/]+)$/, authenticate, async (req, res) => {
		try {
			const packId = req.params[0]
			const { username } = await getUserByReq(req)
			const pack = await getStickerPack(packId)
	...
	router.post(/^\/api\/parts\/shells:stickers\/packs\/([^/]+)\/stickers$/, authenticate, (req, res) => {
	...
	router.delete(/^\/api\/parts\/shells:stickers\/packs\/([^/]+)\/stickers\/([^/]+)$/, authenticate, async (req, res) => {
		try {
			const packId = req.params[0]
	...
	router.post(/^\/api\/parts\/shells:stickers\/install\/([^/]+)$/, authenticate, async (req, res) => {
		try {
			const { username } = await getUserByReq(req)
	...
	router.post(/^\/api\/parts\/shells:stickers\/uninstall\/([^/]+)$/, authenticate, async (req, res) => {
		try {
			const { username } = await getUserByReq(req)
	...
	router.get(/^\/api\/parts\/shells:stickers\/user\/([^/]+)$/, authenticate, async (req, res) => {
		try {
			const { username } = await getUserByReq(req)
	...
	router.post(/^\/api\/parts\/shells:stickers\/favorites\/([^/]+)$/, authenticate, async (req, res) => {
		try {
			const { username } = await getUserByReq(req)
	...
	router.delete(/^\/api\/parts\/shells:stickers\/favorites\/([^/]+)$/, authenticate, async (req, res) => {
		try {
			const { username } = await getUserByReq(req)
	...
	router.post(/^\/api\/parts\/shells:stickers\/recent\/([^/]+)$/, authenticate, async (req, res) => {
		try {
			const { username } = await getUserByReq(req)
