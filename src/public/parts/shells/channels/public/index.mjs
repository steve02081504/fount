...
async function loadChannelStickers() {
	...
	try {
		...
		const collResp = await fetch(`/api/parts/shells:stickers/user/${encodeURIComponent(username)}`, { credentials: 'include' })
...
async function loadChannelMembers() {
	...
	try {
		const resp = await fetch(`/api/parts/shells:channels/${currentChannelId}/members`, { credentials: 'include' })
