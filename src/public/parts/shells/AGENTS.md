...
A fount Shell is a self-contained module that provides a user interface (frontend) and corresponding backend logic. It's crucial to understand how URLs map to the shell's files.

### 1.1. URL Structure: The Colon (`:`) Separator

fount uses a special colon-based URL structure to serve a shell's public static files (HTML, CSS, JS, images). The format is:

`/parts/<partpath>:<filepath_within_public>`

- **`/parts/`**: A constant prefix.
- **`<partpath>`**: The path to the shell, e.g., `shells/my-new-shell`.
...
```tree
/src/public/parts/shells/<your-shell-name>/
│
...
│
├── public/                 # [REQUIRED] All frontend files served via the URL structure above.
...
### 1.3. Key File & Directory Responsibilities
...
2. **`public/` (Frontend Directory)**
   ...
   - **`index.mjs`**: The core frontend logic. It should import and use common
     scripts from `@src/public/pages/scripts/` for tasks like theming
     (`applyTheme`), translation (`initTranslations`), and using pre-built
     components (`setTerminal`).
...
Create the standard directory structure inside the `/src/public/parts/shells/` directory.

```bash
# From fount root directory, create directories for a shell named "my-new-shell"
mkdir -p src/public/parts/shells/my-new-shell/public
mkdir -p src/public/parts/shells/my-new-shell/src
touch src/public/parts/shells/my-new-shell/main.mjs
touch src/public/parts/shells/my-new-shell/public/index.html
touch src/public/parts/shells/my-new-shell/public/index.mjs
touch src/public/parts/shells/my-new-shell/src/endpoints.mjs
```
...
```javascript
// /src/public/parts/shells/my-new-shell/main.mjs
...
Define the server-side logic. API and WebSocket endpoints should use the colon-based path convention to ensure uniqueness.

```javascript
// /src/public/parts/shells/my-new-shell/src/endpoints.mjs
...
Define the UI and connect it to the backend. Note how relative paths are used to reference assets.
...
<html>
  ...
  <body class="flex flex-col h-screen items-center justify-center p-4">
    ...
    ></div>
    <!-- Main script for this page. The path is relative to index.html -->
