...
- `public/`: Frontend assets. `public/llms.txt`: AI-readable docs — **Chinese only**; keep part intro, operational conclusions/guidelines, and every HTTP/WS API with usage. **No UI/layout/keyboard-interaction details** (buttons, dropdowns, tab strip rendering, shortcut keys, placeholder text, bubble alignment, `+`/pill behaviors) — UI mechanics belong in `AGENTS.md`, never `llms.txt`. Also no research notes, design-doc § refs, wire-protocol dumps, or in-process client APIs (those belong in `AGENTS.md` / `decl/`).
...
4. **GetReply identity**: when building `chatReplyRequest` yourself, `User*` must be the local operator; message authors go only in `ReplyTo*` / `chat_log[].uid`. Also set `chat_id` to a stable, channel-unique conversation key (Agent Studio groups and reconstructs history by it; a char recording without it logs an error and is skipped). Details: [chat/session/AGENTS.md](chat/src/chat/session/AGENTS.md) Speaker identity. Platform bots use virtual bridge sessions — never `newGroup` for Discord/Telegram/WeChat chats.
