/**
 * 扫描帖子正文中的群表情 token 并提取 contentHash 引用。
 */
import { EMOJI_TOKEN_RE } from 'fount/public/parts/shells/chat/public/shared/inlineTokenSyntax.mjs'
import { resolveGroupEmojiPostEmbedProvider } from 'fount/public/parts/shells/chat/src/chat/lib/groupEmojiPostEmbedRegistry.mjs'

/**
 * @param {string} text 帖子正文
 * @returns {Array<{ groupId: string, emojiId: string }>} 表情引用列表。
 */
export function scanEmojiTokens(text) {
	const refs = []
	const seen = new Set()
	for (const match of String(text || '').matchAll(EMOJI_TOKEN_RE)) {
		const key = `${match[1]}/${match[2]}`
		if (seen.has(key)) continue
		seen.add(key)
		refs.push({ groupId: match[1], emojiId: match[2] })
	}
	return refs
}

/**
 * best-effort 将帖子中引用的表情写入 CAS 并返回 mediaRefs。
 * @param {string} username 发帖用户
 * @param {string} text 帖子正文
 * @returns {Promise<object[]>} mediaRefs 数组。
 */
export async function buildEmojiMediaRefsForPost(username, text) {
	const { resolveGroupEmojiContent, storeEmojiInCas } = resolveGroupEmojiPostEmbedProvider()
	if (!resolveGroupEmojiContent || !storeEmojiInCas) return []

	const refs = scanEmojiTokens(text)
	/** @type {object[]} */
	const mediaRefs = []
	for (const { groupId, emojiId } of refs)
		try {
			const resolved = await resolveGroupEmojiContent(username, groupId, emojiId)
			let contentHash = resolved?.entry?.contentHash
			if (!contentHash && resolved?.buffer)
				contentHash = await storeEmojiInCas(resolved.buffer)
			if (contentHash)
				mediaRefs.push({ kind: 'groupEmoji', groupId, emojiId, contentHash })
		}
		catch { /* best-effort */ }

	return mediaRefs
}
