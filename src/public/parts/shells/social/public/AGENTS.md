...
- **CSS class names**: page-local — path already says social, so no `social-` prefix on classes. Root `#shell` / `.shell`; icons `.icon` + `.icon-*` (e.g. `icon-content-warning`, `icon-direct-message`, `icon-notification-like`). Full words; no `s-`/`c-`/`app-`. Ready-gate ids/events stay global (`SOCIAL_GATE` / `fount:social-*`).
...
- **Hash routing**: `switchView` writes `#feed`/`#explore`/…/`#drafts`/`#settings`; `applyIncomingNavigation` handles main nav + `#post;entity;postId` detail + `#search;q` / `#search:q` / `?q=` (all open `#searchView`, not inline feed filter). Refreshing restores the current tab.
...
- **Long body fold**: feed/profile/search cards (`openDetail !== false`) collapse markdown code `<details>` by default and clamp tall `.body` (~280px) with `.body-expand`（`social.feed.showMore`/`showLess`）. Post detail keeps full height. `POST_CARD_OPEN_EXCLUDE` includes `summary`/`.body-expand` so toggling code or expand does not navigate to `#post;…`.
...
- **Author hover card**: `lib/profileHover.mjs` → chat `shared/entityProfileHoverCard.mjs` (same `profile_popup` + `paintEntityProfileCard` as click/embedded cards, including tags). Wired once in `init` bootstrap; do not invent a Social-only hover DOM.
- **Entity avatars**: always use chat `shared/entityAvatar.mjs` (`renderAvatarHtml` / `entityAvatarUrl`) + `hashAvatar.mjs` (`customProfileAvatar`); without explicit avatar, draw hash initial letter; do not blindly fetch `files/profile/avatar` or hand-write `charAt(0)`. Composer / reply box must pass `state.viewerProfile` (containing `avatar` + `infoDefaults`), same source as post card `authorProfile`.
...
- Governance menu optimistic UX: `socialWrite.mjs` (`removePostsByAuthor` / `restoreRemovedPosts` / feedShown purge helpers) + `runWrite` failure toasts.
- Hashtag / trending links → `#topic:…` topic view (`loadTopicView`), not search (`#search;` / `#search:`). Playwright must assert `#topicView` / `#topicPostList`. Search Enter / deep links assert `#searchView` / `#searchViewResults` (clear via `#feedSearchClearButton` → `switchView('feed')`).
...
- **Profile header**: bio / post Markdown → chat `shared/trustedMarkdown.mjs` (thin wrappers around `display.renderTrustedPostMarkdown` / `mountMarkdown`); `isTrustedMarkdownAuthor` for self, local char entities (`nodeHash` prefix), declared master (`viewer.ownerEntityHash === author`), and trust list → trusted tier. Remote self-declared `ownerEntityHash` does not elevate. Do not trust remote HTML; do not `escapeHtml` the markdown source.
...
- Viewer identity: `viewerEntityHash()` / `state.viewerEntityHash` (operator; directly imported by frontend modules, no appContext bag).
