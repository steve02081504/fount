...
- **Profile header**: bio 读 `description` / `description_markdown` 源，经 chat `paintEntityProfileBio` / Social `mountMarkdown` 本机安全渲染（`processFountMessageMarkdown`）；勿信对端 HTML、勿 `escapeHtml` 源文本。有 `ownerEntityHash` 时显示「此实体为 xxx 所有」链接（`formatSocialProfileHref`）。Social 时间线不存在导入重签归因问题，无需 attribution mismatch UI。
