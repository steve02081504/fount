...
- **Profile header**: bio / post Markdown → chat `shared/trustedMarkdown.mjs`（`display.renderMarkdown` / `mountMarkdown` 薄包装）；`isTrustedMarkdownAuthor` for self, local agents (nodeHash prefix), owned agents (`author.ownerEntityHash === viewer`), declared master (`viewer.ownerEntityHash === author`), and trust list → trusted tier. Do not trust remote HTML; do not `escapeHtml` the markdown source.
