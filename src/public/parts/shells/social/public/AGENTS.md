...
- **Personal block/hide** (per entity, Chat + Social shared): public `block`/`unblock` timeline events → `personal_block.json` index + reputation propagation; private `hide` → `personal_hide.json` only. **Group kick/ban** remains `denylist.json` group scope + DAG governance (separate).
