...
- Shared: `/scripts/lib/infiniteScroll.mjs` (`data-scroll-sentinel` + `insertBeforeScrollSentinel`). Do not `appendChild` past the sentinel. Rising-edge arm; after replay do not rebind while intersecting. `data-feed-replaying` marks an in-flight loop replay. If loader has its own mutex, bind **after** releasing it.
