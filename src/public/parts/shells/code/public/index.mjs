...
const state = {
	...
	shellMode: false,
	generating: false,
	/** 待发送附件（发送时并入用户消息 files，键为 { name, mime_type, buffer: base64, description }）。 */
	pendingFiles: [],
}
...
function messageActionButton(className, i18nKey, svg, onClick) {
	const button = document.createElement('button')
	button.type = 'button'
	button.className = `code-message-actionbtn ${className}`
...
function startEditEntry(entry, bubble) {
	...
	// aria-label 走 i18n data 属性机制（对象键），语言切换自动更新
	textarea.dataset.i18n = 'code.message.edit.ariaLabel'
	const buttons = document.createElement('div')
	buttons.className = 'code-message-editor-buttons'
...
/**
 * 切换 👍/👎 或记录带原因的反馈（存入 entry.extension.feedback，随会话落盘；仅本地标记）。
 * 不带 content 为切换（同向再点取消）；带 content 为直接记录（原因提交）。
 * 原位更新按钮态，不整页重渲（保持 details 展开与滚动位置）。
 * @param {object} entry - 会话条目。
...
function showFeedbackReason(entry, bubble) {
	...
	textarea.setAttribute('user-content', '')
	// aria-label / placeholder 走 i18n data 属性机制（对象键），语言切换自动更新
...
function renderEntryBubble(entry, { isLast = false } = {}) {
	...
	if (entry.role === 'tool' || entry.role === 'system') {
		...
		// markdown 渲染进独立容器：异步完成后不覆盖同步追加的附件 chip
		const content = document.createElement('div')
...
function updateRegenButtons() {
	...
	for (const el of elements.messages.querySelectorAll('.code-message[data-entry-id]')) {
		...
		if (!regen) continue
		regen.hidden = !(last && String(last.id) === el.dataset.entryId && last.role === 'char' && !state.generating)
	}
}
...
/**
 * 空态布局开关：无条目且未在生成时 composer 垂直居中 + wordmark（工作区选择走下方 workspace pill）。
 * 生成中（entries 仍为空）不算空态——消息流必须保持可见，流式气泡才有容器。
 * @returns {void}
 */
function updateEmptyMode() {
	const empty = !(state.session?.entries?.length || 0)
		&& !(state.generating && generatingSession === state.session)
...
function renderAttachmentPreview() {
	...
	strip.replaceChildren(...state.pendingFiles.map((file, index) => {
		...
		// aria-label 走 i18n data 属性机制，语言切换自动更新（对象键 code.attach.remove）
	...
	strip.hidden = !state.pendingFiles.length
}
...
async function addComposerFiles(files) {
	for (const file of files) {
		...
		state.pendingFiles.push({ name: file.name, mime_type: file.type || 'application/octet-stream', buffer: await blobToBase64(file), description: '' })
	}
	renderAttachmentPreview()
}
...
const composerShell = document.querySelector('.code-composer-shell')
...
composerShell.addEventListener('dragenter', event => {
	...
	event.preventDefault()
	dragDepth++
	composerShell.classList.add('drag-over')
	dropOverlay.hidden = false
})
composerShell.addEventListener('dragleave', () => {
	if (--dragDepth <= 0) {
		dragDepth = 0
		composerShell.classList.remove('drag-over')
		dropOverlay.hidden = true
	}
})
composerShell.addEventListener('dragover', event => event.preventDefault())
 ...
 * 生成中气泡整段保持空白）。可信档（气泡正文）与安全档（StreamRenderer 预览）各一档。
 * @returns {void}
 */
function warmupMarkdownPipeline() {
	void renderMarkdownAsString('', markdownCache)
	void renderMarkdownAsString('', markdownCache, { allowDangerousHtml: false })
}
