...
- PKCE providers bind the upstream-registered localhost port and 302 here (`1455/auth/callback` Codex, `53692/callback` Claude); the port stays bound until `forget` closes the hook. Device flow (GitHub Copilot) does not bind a port.
...
- Config UI: `public/src/oauthDisplay.mjs` exports `renderOauthPanel`. OAuth generators’ `display.mjs` dynamically `import` it and `.then` into `renderOauthPanel` with that generator’s provider id. Logout: `delete cache.oauthState` after `cancelOAuth` so the status poll exits; persist the oauth-less config with `setServiceSourceFile`.
