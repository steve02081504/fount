...
4. **Shared-path value kinds** — every other locale JSON must match `zh-CN` types on keys both trees share (`string` vs object is a hard fail). `update-locales.py` wraps string↔single DOM applicator (`aria-label` / `title` / …) during sync and **exits 1** if any type mismatch remains.
