...
async function executeGeneration(chatid, request, stream, placeholderEntry, chatMetadata) {
	...
	try {
		...
		if (e.name === 'AbortError') {
			...
			console.error(`Generation error for message ${entryId}:`, e)
...
export async function modifyTimeLine(chatid, delta) {
	...
	if (newTimeLineIndex >= chatMetadata.timeLines.length) {
		...
		currentEntry.files = []
		currentEntry.time_stamp = new Date()

		// Set the timeline to this regenerating entry
		chatMetadata.timeLines.push(currentEntry)
		...
		broadcastChatEvent(chatid, {
			type: 'message_replaced',
			payload: { index: chatMetadata.chatLog.length - 1, entry: await currentEntry.toData(chatMetadata.username) },
		})

		// Start generation
		const request = await getChatRequest(chatid, charname)
		const stream = StreamManager.create(chatid, currentEntry.id)
		executeGeneration(chatid, request, stream, currentEntry, chatMetadata)
	} else {
		// This is a simple timeline switch
