...
export const actions = {
	...
	'set-char-frequency': ({ chatId, charName, frequency }) => {
		if (!chatId || !charName || typeof frequency !== 'number') throw new Error('Chat ID, character name, and frequency are required.')
	...
	'delete-message': ({ chatId, index }) => {
		if (!chatId || typeof index !== 'number') throw new Error('Chat ID and message index are required.')
		return deleteMessage(chatId, index)
	},
	'edit-message': ({ chatId, index, newContent }) => {
		if (!chatId || typeof index !== 'number' || !newContent) throw new Error('Chat ID, message index, and new content are required.')
		return editMessage(chatId, index, newContent)
	},
	'modify-timeline': ({ chatId, delta }) => {
		if (!chatId || typeof delta !== 'number') throw new Error('Chat ID and delta are required.')
