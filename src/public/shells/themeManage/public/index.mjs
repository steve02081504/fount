...
async function fetchCustomThemes() {
	const res = await fetch('/api/shells/themeManage/list')
	if (res.ok) {
...
async function handleThemeApply(id, isCustom) {
	if (isCustom) {
		...
		const url = `/api/shells/themeManage/theme/${id}`
...
async function handleDelete(id) {
	if (!await confirmI18n('themeManage.deleteConfirm', { id })) return
	await fetch(`/api/shells/themeManage/theme/${id}`, { method: 'DELETE' })
...
async function handleClone(id, isCustom) {
	let css = ''
	if (isCustom) {
		const res = await fetch(`/api/shells/themeManage/theme/${id}`)
	...
	const newId = await promptI18n('themeManage.newThemeName', { id }, `copy-${id}`)
	if (!newId) return
	...
	await fetch('/api/shells/themeManage/save', {
		method: 'POST',
...
`
			...
			const res = await fetch('/api/shells/themeManage/save', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
