...
async function handleFormSubmit(event) {
	...
	try {
		...
		responseMessage.textContent = result.message || geti18n('eazynew.alerts.success.generic')
		responseMessage.classList.add('alert', 'alert-success')
		form.reset()
		await loadTemplateUI()
	} catch (error) {
		responseMessage.textContent = geti18n('eazynew.alerts.error', { message: error.message })
...
async function main() {
	applyTheme()
	await initTranslations('eazynew')
