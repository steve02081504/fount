...
function getUserTemplatesPath(username) {
	return path.join(getUserDictionary(username), 'shells', 'eazynew', 'parts')
}
...
export function setEndpoints(router) {
	router.get('/api/shells/eazynew/templates', authenticate, async (req, res) => {
		try {
	...
	router.get('/api/shells/eazynew/template-html', authenticate, async (req, res) => {
		try {
			const { username } = await getUserByReq(req)
	...
	router.post('/api/shells/eazynew/create', authenticate, async (req, res) => {
		try {
			const { username } = await getUserByReq(req)
			...
			res.status(201).json({ message: `Part '${partName}' created successfully!` })
		} catch (error) {
