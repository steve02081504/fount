...
async function fetchVersionInfo() {
	try {
		...
		els.localVersion.textContent = localVer
		debugData.version.local = localVer

		// Remote Version
		const remoteRes = await fetch('https://api.github.com/repos/steve02081504/fount/commits/master')
		const remoteData = await remoteRes.json()
		const { sha: remoteVer } = remoteData
		els.remoteVersion.textContent = remoteVer.substring(0, 7) // Display short SHA
		debugData.version.remote = remoteVer

		// Compare
		if (localVer === remoteVer) {
			els.versionIndicator.className = 'badge badge-lg badge-success gap-2'
			els.versionIndicator.dataset.i18n = 'debug_info.versionStatus.upToDate'
		} else {
			els.versionIndicator.className = 'badge badge-lg badge-error gap-2'
			els.versionIndicator.dataset.i18n = 'debug_info.versionStatus.outdated'
		}
	} catch (error) {
		console.error('Version check failed:', error)
		els.versionIndicator.className = 'badge badge-lg badge-warning gap-2'
		els.versionIndicator.dataset.i18n = 'debug_info.versionStatus.checkFailed'
	}
	i18nElement(els.versionIndicator)
}
...
async function fetchSystemInfo() {
	try {
		...
		els.systemInfoTable.innerHTML = ''
		els.systemInfoTable.appendChild(await renderTemplate('system_info_table', { rows }))

		// Render Backend Connectivity
		debugData.connectivity.backend = connectivity
		els.backendChecks.innerHTML = ''
		els.backendChecks.appendChild(await renderTemplate('connectivity_list', { checks: connectivity }))

	} catch (error) {
		console.error('System info fetch failed:', error)
		els.systemInfoTable.innerHTML = '<tr><td colspan="2" class="text-error text-center" data-i18n="debug_info.systemInfo.failed"></td></tr>'
		i18nElement(els.systemInfoTable)
	}
}
...
async function checkFrontendConnectivity() {
	...
	els.frontendChecks.innerHTML = ''
	els.frontendChecks.appendChild(await renderTemplate('connectivity_list', { checks }))
	...
	for (const check of checks) {
		...
		try {
			await fetch(check.url, { method: 'HEAD', mode: 'no-cors' }) // no-cors for opaque check
			status = 'ok'
...
Status: ${els.versionIndicator.textContent}
