...
const configTemplate = {
	...
	model: 'llama3',
	system_prompt_at_depth: 1,
	model_arguments: {
		// 根据 Ollama API 文档设置默认参数
		// 'mirostat': 0,
		// 'mirostat_eta': 0.1,
		// 'mirostat_tau': 5.0,
		// 'num_ctx': 4096,
...
async function GetSource(config) {
	const ollama_client = new ollama.Ollama({ host: config.host })
	/** @type {AIsource_t} */
	const result = {
		type: 'text-chat',
		 ...
		 */
		Call: async prompt => {
			const response = await ollama_client.generate({
				model: config.model,
				prompt,
				stream: false,
		...
		tokenizer: {
			...
			get_token_count: async prompt => {
				if (!prompt) return 0
				try {
					const response = await ollama_client.encode({ model: config.model, prompt })
