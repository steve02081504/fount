 ...
 * @param {string} [options.placeholder] - 输入占位符。
 * @param {string} [options.hint] - 底部按键提示。
 * @returns {{ inputElementement: HTMLTextAreaElement, completionsElementement: HTMLElement, syncInputView: () => void, setBusy: (busy: boolean) => void, focus: () => void }} REPL UI 句柄。
 */
export function mountReplPanel(container, { placeholder = '', hint = '' } = {}) {
	...
	busyElement.className = 'repl-frame-busy'
	busyElement.textContent = '⋯'
	frameTop.appendChild(busyElement)
	...
	gutterElement.className = 'repl-gutter'

	const stackElement = document.createElement('div')
	stackElement.className = 'repl-stack'

	const highlightElement = document.createElement('pre')
	highlightElement.className = 'repl-highlight'
	highlightElement.setAttribute('aria-hidden', 'true')

	const inputElement = document.createElement('textarea')
	inputElement.id = 'repl-input'
	inputElement.className = 'repl-input-edit'
	inputElement.rows = 1
	...
	stackElement.append(highlightElement, inputElement)
	editor.append(gutterElement, stackElement)
	editorWrap.appendChild(editor)
	inputArea.appendChild(editorWrap)

	const completionsElement = document.createElement('ul')
	completionsElement.id = 'repl-completions'
	completionsElement.className = 'repl-completions hidden'
	inputArea.appendChild(completionsElement)
	...
	hintElement.className = 'repl-frame-hint'
	hintElement.textContent = hint
	frameBottom.appendChild(hintElement)
	...
	function updateGutter(lineCount) {
		...
		for (let i = 0; i < count; i++) {
			...
			gutterElement.appendChild(line)
		}
	}
	...
	function syncInputView() {
		...
		highlightElement.innerHTML = lines.map(highlightLine).join('\n') || '&nbsp;'
		inputElement.style.height = '0'
		inputElement.style.height = `${Math.max(inputElement.scrollHeight, stackElement.offsetHeight)}px`
	}
	...
	function setBusy(busy) {
		busyElement.classList.toggle('active', busy)
	}
