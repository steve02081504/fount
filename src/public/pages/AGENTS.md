...
- **`svg-theme-ignore`**: boolean attr on intentional low-contrast brand SVGs (e.g. wordmarks) to skip the page-watch SVG theme contrast check. Put it on the source `<img>` when `svgInliner` will inline it — the inliner copies attributes onto the resulting `<svg>`.
