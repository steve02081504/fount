...
- **`infiniteScroll.mjs`**: `ensureScrollSentinel` / `insertBeforeScrollSentinel` / `bindInfiniteScroll` / `disconnectInfiniteScroll`. Sentinel carries `data-scroll-sentinel`; always append via `insertBeforeScrollSentinel` (or equivalent) so it stays last. Rising-edge arm; after replay do not rebind while intersecting.
