...
- **`components/folderBrowser.mjs`**: `openFolderBrowser({ browse, onSelect, dirsOnly?, initialWorkspace?, onError? })` — shared folder/file picker dialog (template at `/scripts/features/templates/folder_browser.html`, i18n under `util.folderBrowser.*`, CSS injected at import). **`browse(path, workspace?)` is required** (returns `{ path, entries, quickAccess? }`); `onSelect(path)` is invoked after the dialog closes (select button, or clicking a file when `dirsOnly` is false); rejections route to `onError`; `dirsOnly` hides files. The dialog opens **immediately with a `util.folderBrowser.loading` placeholder** and renders when browse resolves — root views can build slow quick-access lists, a blocking open would delay the popup. Entry buttons carry `user-content` (dir/file names are user data, the locale-script scan must skip them). Path input doubles as a filter (last segment, ↑/↓ highlight, Enter opens); editing the path's directory part clears the selection and Enter jumps. Reused e.g. by the code shell workspace picker (`dirsOnly: true`).
- **`components/selectionController.mjs`**: `createSelectionController({ getOrderedIds, onChange, plainClick, shiftRange })` — list multi-select state machine (Shift range / Ctrl·⌘ toggle / select-all / mode / `reconcile` hidden ids). `plainClick: 'toggle' | 'single'` picks gist-style toggle-in-selection-mode vs chat-sidebar single-select; `shiftRange: 'add' | 'replace'` picks additive vs replacing range. `bindSelectionShortcuts(controller, { onEscape?, canSelectAll?, target? })` wires Ctrl/⌘+A / Esc and ignores editable targets. DOM sync stays in `onChange`. Consumers: gist list, chat sidebar (`hub/groupSelection.mjs`).
...
- **`userActivity.mjs`**: tracks `{ lastInteractionAt, hasFocus }` (focus/blur/visibility + throttled interaction events) and posts it to the SW (`USER_ACTIVITY_UPDATE`). SW suppresses a system notification only when a matching-shell page (`pathname`) is focused **and** interacted within 4s; it always forwards a page-global `fount-notification` event so each shell updates its badge (notification-driven, not gated on the active page). Page-active notifications play `notificationSound.mjs` (Web Audio chime); on playback failure `base.mjs` asks the SW to `SHOW_NOTIFICATION_FALLBACK` (system notification). Wire details live in `service_worker.mjs` `routeNotification`.
...
- **Standalone document export** lives in the gist shell now (`/parts/shells:gist/src/standaloneDocument.mjs`, browser-absolute imports): `renderMarkdownAsStandaloneDocument` / `wrapStandaloneMarkdownDocument` — offline full HTML for Chat/Social/Code download/share/drag. Filenames via `fileNameFromHtmlTitle` / `downloadHtmlDocument`. Cross-shell browser imports use the `/parts/shells:gist/src/…` URL; Deno resolves `/scripts/…` via the deno.json import map.
...
- **`i18n.mjs`**: Sole public entry. Call `initTranslations()` early. Switch UI with **`setLanguage(string[])`**; raw bundle: **`loadLocaleData(string[])`**. Prefer these over ad-hoc fetch. `data-i18n`, `geti18n`, `setElementI18n`, `primaryLocale()`. Missing keys → `console.warn('[i18n:missing] …')` (Playwright hard-fails). Params / placeholders / switch leaves / aria-label: [docs/i18n-notes.md](docs/i18n-notes.md). Page pages: pass the pageid root (`initTranslations('code')`) — `applyTranslations` resolves `<title>`/description meta from `${pageid}.title|.description` every language change.
  - `onLanguageChange(callback)` **fires the callback immediately upon registration** — register after `initTranslations` resolves (e.g. inside `boot`), never at module top level, or `[i18n:missing]` warns while the bundle is still empty.
  - `data-i18n="<object key>"` (a key resolving to an object) fills attributes from child keys: `<key>.aria-label`, `.placeholder`, `.title`, `.value`, `.textContent`… — declare those child keys instead of hardcoding empty `aria-label=""`.
  - `geti18n()` textContent is **not** re-rendered by `setLanguage`; JS-rendered UI chrome needs its own `onLanguageChange` rerender (register after init). Static `data-i18n` nodes are handled by the i18n MutationObserver automatically.
...
- **`prompt-content`**: boolean attr on prompt / model / plugin generated content (tool logs, structured tool cards, sub-agent transcripts). Same locale-scan subtree skip as `user-content`; use it instead of `user-content` when the text is prompt-side rather than user/agent chat text.
- **`data-app-shell`**: on `<body>` of an exactly-one-screen app shell (viewport-height layout with internal scrollers). Enables page watch `[test:viewport]`, which appends a hidden extension-like overlay and fails the page if it pushes the document past the viewport (a nearly-unscrollable page scrollbar). Such a shell must clip `body` (`overflow: clip`). Do not declare it on long document pages that scroll the document.
- **`aria-ignore`**: value **must** be a GitHub issue URL. Policy: [test AGENTS](../../scripts/test/AGENTS.md) Operator tools.
- **`svg-theme-ignore`**: boolean attr on intentional low-contrast brand SVGs (e.g. wordmarks) to skip the page-watch SVG theme contrast check. Put it on the source `<img>` when `svgInliner` will inline it — the inliner copies attributes onto the resulting `<svg>`. Not needed for self-backing SVGs that paint their own opaque full-frame backdrop (e.g. the square fount logo) — the checker skips those automatically (unreliable measurement, backing is the SVG's own interior).
