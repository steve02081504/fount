...
Icon-only controls: locale `{ title, aria-label }` ([locale-edits.md](../../locales/docs/locale-edits.md)); string keys on icon parents wipe children. Never hardcode English `aria-label` as a fallback — page watch requires Han on zh pages and Japanese script on ja pages; use `data-i18n` objects with `aria-label`. Enforced at runtime: before a leaf string (or `textContent`/`innerHTML` applicator) replaces an element subtree, the i18n lib warns `[i18n:clobber]` when it contains tags outside `div/p/br/code/span/a` (still writes through); Playwright fixtures hard-fail on that warning — fix by switching to an object key with `title`/`aria-label`.
