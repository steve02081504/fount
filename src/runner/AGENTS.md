...
An explicit `FOUNT_DIR` wins; otherwise use the launcher on PATH or the platform default. Reuse a valid tree even when PATH registration is missing. Reject an occupied unknown target before network or package operations. Clone retries / ZIP extraction use isolated temporary directories; publish all contents (including dotfiles) into an empty target without replacing its directory inode. Preserve the caller's working directory and relative CLI arguments. Shared PowerShell target checks must remain outside `#_if PSScript` so the compiled runner retains them.

## First install (no valid tree at the target)
...
1. Install git/curl/unzip as needed, stage the clone or ZIP download, then copy/move its contents into `FOUNT_DIR`. Errors here are English.
...
4. `N` or unavailable console input: exit without starting fount or removing downloaded files / existing data. Uninstallation requires an explicit `fount remove` command.
...
No `data/config.json` (except `remove`): `ensure_fount_config` / `Ensure-FountConfig`, then the original command. Same EULA + 8930 + wait page; refusal or missing consent stops startup and preserves the installation. `cmd_open` opens `wait?cold_bootting=true` only when `FOUNT_INSTALL_WAIT` is unset. See [path AGENTS](../../path/AGENTS.md).
