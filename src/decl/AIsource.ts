 ...
 * @description 定义了 AI 数据源的基本结构，用于与不同类型的 AI 模型进行交互。
 */
export class AIsource_t<InputType, OutputType> {
	/**
	 * @description AI 数据源的文件名。
	 */
	filename: string
	/**
	 * @description AI 数据源的类型，例如 'text-chat'。
	 */
	type: 'text-chat' | string
	/**
	 * @description AI 数据源的详细信息。
	 */
	info: info_t
	/**
	 * @description 指示该 AI 数据源是否为付费服务。
	 */
	is_paid: boolean
	/**
	 * @description 用于存储扩展功能的对​​象。
	 */
	extension: object

	/**
	 * @description 卸载 AI 数据源并释放资源。
	 * @returns {Promise<void>}
	 */
	Unload?: () => Promise<void>
	/**
	 * @description 调用 AI 数据源进行处理。
	 * @param {InputType} prompt - 输入数据。
	 * @returns {OutputType} - 处理后的输出数据。
	 */
	Call: (prompt: InputType) => OutputType
	/**
	 * @description AI 数据源支持的接口。
	 */
	interfaces: {
		/**
		 * @description 信息接口，用于更新 AI 数据源的信息。
		 */
		info?: {
			/**
			 * @description 更新 AI 数据源的本地化信息。
