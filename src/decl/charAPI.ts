...
export class CharAPI_t {
	...
	interfaces: {
		...
		social?: {
			/**
			 * 被 @ 提及（mention 目标为 128 位 entityHash，任意本机或远程 P2P 实体）。
			 * @param {SocialMentionEvent} event - 提及上下文。
			 * @returns {Promise<SocialHandlerResult>} - 回复正文；null 跳过。
			 */
			OnMention?: (event: SocialMentionEvent) => Promise<SocialHandlerResult>
			/**
			 * 有新关注者。
			 * @param {SocialFollowEvent} event - 关注事件。
			 * @returns {Promise<void>}
			 */
			OnFollow?: (event: SocialFollowEvent) => Promise<void>
			/**
			 * 所关注实体发布了新动态（无默认行为，需显式实现）。
			 * @param {SocialFollowerUpdateEvent} event - 新帖上下文。
			 * @returns {Promise<SocialHandlerResult>}
			 */
