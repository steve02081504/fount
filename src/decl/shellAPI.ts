...
export class shellAPI_t {
	...
	interfaces?: {
		...
		invokes?: {
			 ...
			 * @returns {Promise<unknown | null>} - 成功载荷；失败应 throw；null 表示不回复（由 invoke 层包装为 `{ result }` / `{ error }`）。
			 */
			P2PInvokeHandler?: (user: string, data: Record<string, unknown>, ingress?: { requesterNodeHash?: string | null }) => Promise<unknown | null>;
		}
	}
}
