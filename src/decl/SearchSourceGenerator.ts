 ...
 */
export class SearchSourceGenerator_t {
	/**
	 * 搜索数据源生成器的详细信息。
	 */
	info: info_t
	...
	interfaces: {
		/**
		 * 信息接口，用于更新搜索数据源生成器的信息。
		 */
		info?: {
			/**
			 * 更新搜索数据源生成器的本地化信息。
			 * @param {locale_t[]} locales - 本地化信息数组。
			 * @returns {Promise<info_t>} - 更新后的搜索数据源生成器信息。
			 */
		...
		serviceGenerator: {
			...
			/**
			 * 根据配置获取搜索数据源实例。
			 * @param {any} config - 搜索数据源的配置。
			 * @param {{ username: string, SaveConfig: () => Promise<void> }} args - 包含用户名和保存配置函数的参数。
			 * @returns {Promise<SearchSource_t>} - 搜索数据源实例。
			 */
			GetSource: (config: any, args: { username: string, SaveConfig: () => Promise<void> }) => Promise<SearchSource_t>
		}
	}
}
