...
- **Logs**: `fount log` — main-process console via `localhost` (not `127.0.0.1`). Check before guessing from browser 404s. Interactive VT: icon_anime `intro` on start, `farewell` on `on_shutdown`; viewer owns `exitSignal`, wires `icon.signal` into it; non-TTY skips logo (player-gated). Foreground server on a real TTY (`needs_output`, e.g. Termux / in_container keepalive → server only): same icon session in `src/server/index.mjs` (like log_viewer); `dismiss` when `started`, `farewell` on shutdown (also covers mid-intro exit on `already_running`). Standalone TUI: `fount logo` / `fount logo watch`.
...
- **path CLI**: thin entries `path/fount.{ps1,sh}` dispatch to `path/src/cmd/<name>.*` via inline `Test-Path` + dot-source in `index.{ps1,sh}`; no early-passthrough branch in entries. Each cmd loads its own deps (`require`); `path/src/**/*.ps1` exports use `function script:` (esh-style explicit scope). Full install/runtime via `bootstrap_full`, server via `bootstrap_server`, `remove` uses `require_mid` + `source_uninstall_hooks`. Same logic isomorphic across `foo.{ps1,sh}`; platform-only under `path/src/win/` or `unix/`. Shared helpers: `in_container`, `run_with_updates`, `trap_terminal_teardown`, `handle_docker_passthrough`, `check_temp_guard`, `sed_escape`. Termux: `env.sh` sets `LANG` from `getprop persist.sys.locale` at CLI start (before i18n); `termux_ensure_sensor_api` (in `unix/termux.sh`) installs `termux-api` for `termux-sensor` when missing on `fount logo` / `log` / `server` (`termux_run_setup`), tracked in `auto_installed_system_packages` for uninstall. `remove` scans `**/*.uninstall.<level>.*` under `path/src`, runs highest level first; level `0` deletes the install tree. Updates: `update_fount_and_deno` (sync) / `update_fount_and_deno_background` (backgrounds sync after `data/installer/deno_upgraded`); `bootstrap_server` uses background; keepalive retries and exit-131 call sync before restart. `fount update` is the sync CLI (also how PS Start-Job re-enters); plain update refreshes only the current branch via one-shot `git_fetch_remote_branch` (does not widen `remote.origin.fetch`). `fount update <branch>` checks out that branch and removes `.noupdate`; unknown names `ls-remote` once then the same one-shot fetch. `fount update <sha>` detaches at that commit and creates `.noupdate`. If the current upstream is confirmed gone on origin (not a network error), fall back to tracking `master`.
- **path CI** (`test_running.yaml` job `path-cmd-smoke`): CI-only — `.github/path-ci/install-hooks.sh` swaps `src/server/index.mjs` / `src/log_viewer/index.mjs` with stubs; `run-smoke.{sh,ps1}` exercises `server` / `background` / `log` / `reboot` / `init` (install); `FOUNT_CLICK` coverage via child `smoke-fount-click.ps1` (`function script:` stubs + capture file — not `global:`). Windows invokes the helper with `powershell` (5.1); non-Windows uses `pwsh`. Windows → `Start-WTfountCmd`; non-Windows → `handle_unix_passthrough` → bash, where `index.sh` `unset`s `FOUNT_CLICK` (never loads `win/wt`). `restore-hooks.sh` on exit. Same workflow also runs remote init → `remove` (`test-fount`).
- **path `Start-Process -ArgumentList`**: always a single string (`Get-FountPs1ArgumentList` / `Get-WTfountCmd`), never a PowerShell array — nested/empty elements bind as `""` and throw. `Get-FountPs1ArgumentList` escapes each `@args` token (spaces / quotes / empty) then joins. Spawn failure: `-ErrorAction Stop` + try/catch → `exit 1` (PS often still exits 0 after a displayed error).
