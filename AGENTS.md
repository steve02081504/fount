...
- **Testing**: `fount test` — self-contained, no running server. Deno self-update is **kernel-owned** — the path CLI no longer upgrades per invocation; the detached kernel checks on startup and whenever the run queue drains (throttled by `data/installer/deno_upgraded.test`), and a successful upgrade restarts only the kernel. `FOUNT_TEST_SKIP_DENO_UPGRADE` / `FOUNT_TEST_DENO_UPGRADE_INTERVAL` are read by the **kernel process**, so setting them on a `fount test` call does not reach an already-running detached kernel — use `fount test --kernel reboot` to apply them to a fresh kernel. Default: one plan of imperfect (incl. fresh noisy) ∪ outdated; a failure only blocks dependents; never full-repo unless `--all`. Selectors: `manifest` / `manifest:suite` / `manifest:suite:subtest`. Live display: `fount test --watch`; agent-friendly non-TTY output is `plain` (default) / `FOUNT_TEST_OUTPUT=json` (`--json`). See [src/scripts/test/AGENTS.md](src/scripts/test/AGENTS.md). Frontend (Playwright) tests reuse the **system Chrome/Edge** (`resolveBrowserUseOptions`); never `playwright install` locally — a stray `ms-playwright` dir is reported as a cleanup leak (exit 3). Browser-only code is tested **only** in `test/frontend` (the `modulePage` fixture makes in-browser logic tests cheap); framework `deno test` children explode on DOM-global assignment (`no_dom_shim` preload) — never reintroduce happy-dom/linkedom bridges.
- **Flaky tests are bugs**: a test that passes sometimes and fails sometimes (race, timeout, ordering, port reuse, clock, fs timing) must be **investigated and fixed**, not retried or skipped. The only acceptable excuse is a cause software genuinely cannot control (e.g. a remote service being down, or cosmic-ray-level memory corruption) — document that explicitly; "it's flaky" alone is never an excuse. Reproduce by re-running the suite in isolation and under load, find the interleaving/timeout, then fix the code or the test's synchronization — never paper over with sleeps or `--retry`.
...
- **Live eval**: `fount eval "<js>"` / `fount eval -f <file>` (or pipe code via stdin) runs it inside the running server and prints console output + result — inspect in-process state (bot caches, hook flags) without a restart. Both platforms call the shared WS client `src/scripts/eval.mjs` (`path/src/cmd/eval.{ps1,sh}`). The server streams each `eval_output` as it is produced and finishes with `eval_result`, rendering text server-side when the client asks for `text: true` (local-trust, same CSRF rules as the browser REPL); the client lazily reads `fountConsole.path.eval.*` only when it must report an error (a missing key degrades to the key name, like path `Get-I18n`). Dynamic `import()` needs an absolute `file:///…` URL; bare specifiers resolve as `node:`.
...
- **Graceful shutdown**: `process.exit` is replaced by `on-shutdown`'s `shutdown()`, which `await`s every registered hook before the real exit. One hook that never resolves freezes the process mid-exit: tray "restart" / `fount reboot` / shutdown look like no-ops and the process keeps serving. Never `await` a long-running loop as part of startup (see `wechatbot` `startBot`) — resolve startup and run the loop in the background.
...
| Agent Studio / sub-agent / generation history | [src/public/parts/shells/agent_studio/AGENTS.md](src/public/parts/shells/agent_studio/AGENTS.md) |
