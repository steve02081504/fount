...
- **Lint**: `eslint --fix --quiet` (no `npx`). No logging unless error/warning. `jsdoc/require-jsdoc` covers re-exports — write a real one-liner (re-export 非常规，注释说明正常)；不要留空 `/** */` 桩。
...
- **Logs**: `fount log` — main-process console via `localhost` (not `127.0.0.1`). Check before guessing from browser 404s. Interactive VT: `createIconAnime` intro on start, farewell on `on_shutdown`; non-TTY skips logo. Standalone TUI: `fount logo` / `fount logo watch`.
