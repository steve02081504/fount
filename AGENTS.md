...
- **path CLI**: thin entries `path/fount.{ps1,sh}` dispatch to `path/src/cmd/<name>.*` via inline `Test-Path` + dot-source in `index.{ps1,sh}`; no early-passthrough branch in entries. Each cmd loads its own deps (`require`); `path/src/**/*.ps1` exports use `function script:` (esh-style explicit scope). Full install/runtime via `bootstrap_full`, server via `bootstrap_server`, `remove` uses `require_mid` + `source_uninstall_hooks`. Same logic isomorphic across `foo.{ps1,sh}`; platform-only under `path/src/win/` or `unix/`. Shared helpers: `in_container`, `run_with_updates`, `trap_terminal_teardown`, `handle_docker_passthrough`, `check_temp_guard`, `sed_escape`. `remove` scans `**/*.uninstall.<level>.*` under `path/src`, runs highest level first; level `0` deletes the install tree. Updates: `update_fount_and_deno` (sync) / `update_fount_and_deno_background` (backgrounds sync after `data/installer/deno_upgraded`); `bootstrap_server` uses background; keepalive retries and exit-131 call sync before restart. `fount update` is the sync CLI (also how PS Start-Job re-enters); `fount update <branch>` checks out that branch and removes `.noupdate`; `fount update <sha>` detaches at that commit and creates `.noupdate`.
...
- **path `Start-Process -ArgumentList`**: always a single string (`Get-FountPs1ArgumentList` / `Get-WTfountCmd`), never a PowerShell array — nested/empty elements bind as `""` and throw. Spawn failure must `exit 1` explicitly (PS often still exits 0 after a displayed error).
