...
- **path CLI**: thin entries `path/fount.{ps1,sh}` dispatch to `path/src/cmd/<name>.*` via file routing (`. $FountCmdRoute` at script scope / `cmd_*` on sh); no early-passthrough branch in entries. Each cmd loads its own deps (`require` / `RequireMany`); `path/src/**/*.ps1` exports use `function script:` (esh-style explicit scope — plain dot-source in `Require`). Full install/runtime via `bootstrap_full` / `Invoke-FountBootstrapFull`, server via `bootstrap_server` / `Invoke-FountBootstrapServer`, `remove` uses runtime modules only. Same logic as isomorphic `foo.{ps1,sh}`; platform-only under `path/src/win/` or `unix/`. Shared sh helpers: `in_container`, `run_server_with_updates`, `trap_terminal_teardown`, `sed_escape`. `remove` scans `**/*.uninstall.<level>.*` under `path/src`, runs highest level first; level `0` deletes the install tree.
