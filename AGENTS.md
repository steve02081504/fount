...
- **Live eval**: `fount eval "<js>"` / `fount eval -f <file>` (or pipe code via stdin) runs it inside the running server through `/ws/eval` (`async_eval`, sandbox `{ config }`) and prints console output + result — inspect in-process state (bot caches, hook flags) without a restart. Dynamic `import()` needs an absolute `file:///…` URL; bare specifiers resolve as `node:`.
