...
- **I18n**: only edit `src/public/locales/zh-CN.json`; `update-locales.py` syncs other langs (runs on `master` / GitHub Actions only; feature branches skip — PR CI will sync). Backend content locale: `localesForUser` / `primaryLocaleForUser` (`@src/scripts/locale.mjs`); fall back to `en-UK` when the user has no preference. Frontend: `primaryLocale()` (`@src/public/pages/scripts/i18n`).
