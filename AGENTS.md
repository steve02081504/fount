...
- **path CI** (`test_running.yaml` job `path-cmd-smoke`): CI-only — `.github/path-ci/install-hooks.sh` swaps `src/server/index.mjs` / `src/log_viewer/index.mjs` with stubs; `run-smoke.{sh,ps1}` exercises `server` / `background` / `log` / `reboot` / `init` (install); Windows pwsh smoke also covers `FOUNT_CLICK` → `Start-WTfountCmd` (session `Start-Process` stub). Non-Windows pwsh clears `FOUNT_CLICK` and `handle_unix_passthrough` → bash — never loads `win/wt`. `restore-hooks.sh` on exit. Same workflow also runs remote init → `remove` (`test-fount`).
