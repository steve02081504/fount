...
- **IDs / case**: hex、entityHash、eventId 等键原样比较，勿用 `.toLowerCase()` / 便利 `normalizeHex64` 洗本地数据；正文搜索用 `RegExp(..., 'i')`，勿预降 haystack。`normalizeHex64` 仅留验签域、确定性协议 ID、不可信入站鉴权。JSDoc 已是 `string` 时勿再 `String(x || '')`。倒排索引进词键例外见 [search/AGENTS.md](src/scripts/search/AGENTS.md)。
...
- **Logs**: `fount log` — main-process console via `localhost` (not `127.0.0.1`). Check before guessing from browser 404s. Selector one-shot (no intro/farewell, print & exit): `fount log error:5` / `error+warn:10` / `:20` / `5` — see `src/log_viewer/selector.mjs`. Interactive VT logo on log viewer / foreground server TTY: [imgs/icon_anime/AGENTS.md](imgs/icon_anime/AGENTS.md). Standalone: `fount logo` / `fount logo watch`.
