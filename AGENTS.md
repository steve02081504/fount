...
- **Lint**: `eslint --fix --quiet` (no `npx`). No logging unless error/warning. `jsdoc/require-jsdoc` covers re-exports — write a real one-liner; do not leave empty `/** */` stubs.
...
- **Logs**: `fount log` — main-process console via `localhost` (not `127.0.0.1`). Check before guessing from browser 404s. Interactive VT: icon_anime `intro` on start, `farewell` on `on_shutdown`; exit via sticky `icon.signal`; non-TTY skips logo (player-gated). Standalone TUI: `fount logo` / `fount logo watch`.
