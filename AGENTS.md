...
- **Logs**: `fount log` — main-process console via `localhost` (not `127.0.0.1`). Check before guessing from browser 404s. Interactive VT: `createIconAnime` intro on start, farewell on `on_shutdown`; non-TTY skips logo.
