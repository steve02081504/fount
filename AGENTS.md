...
- **path CI** (`test_running.yaml` job `path-cmd-smoke`): CI-only — `.github/path-ci/install-hooks.sh` swaps `src/server/index.mjs` / `src/log_viewer/index.mjs` with stubs; `run-smoke.{sh,ps1}` exercises `server` / `background` / `log` / `reboot` / `init` (install); pwsh smoke also dot-sources `FOUNT_CLICK` → `Start-WTfountCmd` with a session `Start-Process` stub; `restore-hooks.sh` on exit. Same workflow also runs remote init → `remove` (`test-fount`).
