...
- **path CLI**: thin entries `path/fount.{ps1,sh}` dispatch to `path/src/` — same logic as isomorphic `foo.{ps1,sh}`; platform-only under `path/src/win/` or `unix/`. Load via `fount_require` (sh) / `. $FountRequireMany mod1 mod2` (ps1). Shared sh helpers: `fount_in_container`, `run_server_with_updates`, `fount_trap_terminal_teardown`, `fount_sed_escape`. `remove` scans `**/*.uninstall.<lv>.*` under `path/src`, runs highest lv first; lv `0` deletes the install tree.
