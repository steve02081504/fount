...
fount는 다양한 파츠를 로드하여 서비스를 제공하는, 프로그래밍 가능한 표준화·모듈화·커스터마이징 에이전트 런타임 플랫폼입니다.  
...
- API만 설정하면 기본 캐릭터 ZL-31로 대화를 통해 파츠 설정·생성 등 **모든 사용자 작업** 완료—학습·온보딩 불필요

#### fount를 쓰지 않는 이유

- 숙달 비용이 다소 높고, 코드 지식 필요
- 일부 커뮤니티 파츠에 악성 코드가 있을 수 있어 판단·선별 필요
...
- 채팅 기록이 클라우드에만 저장되어 내보내기·이전이 안 되는 것을 괜찮을 때
- 광고를 괜찮을 때
...
## 🚀 효율성과 엔지니어링: 직장인, 개발자, 괴짜들을 위해 구축됨
...
- **워크플로우 캡슐화 및 배포**: 생성한 역할(에이전트)을 데스크톱으로 직접 끌어다 놓아 공유 가능한 파일을 생성합니다. 당신의 프롬프트 엔지니어링과 워크플로우 로직을 동료나 커뮤니티에 쉽게 배포할 수 있습니다.
  ![이미지](https://github.com/user-attachments/assets/5e14fe6e-2c65-492a-a09f-964c1e8ab9e0)

### 3. 실시간 코드 실행 환경

더 이상 단순히 마크다운 하이라이팅이 아닙니다. fount에서 역할(에이전트)이 보내는 코드 블록은 **살아있습니다**.
...
- **언제 어디서든 원활한 대화**
  컴퓨터에서 시작한 대화는 휴대폰이나 태블릿에서 원활하게 이어질 수 있습니다. fount는 당신의 대화를 동기화하여, 당신이 어디에 있든지 당신의 역할(에이전트)과 긴밀하게 연결되어 있도록 보장합니다.

- **표현력이 풍부하고 몰입감 있는 채팅**
  HTML의 힘을 활용하여, fount는 역할(에이전트)이 리치 텍스트, 이미지, 심지어 대화형 요소를 통해 자신을 표현할 수 있게 하여, 모든 대화를 생생하고 심오하게 만듭니다.

- **생각의 모임: 기본 그룹 채팅**
  여러 역할을 동일한 대화에 초대하여, 업무상의 브레인스토밍이든 역할 간의 스토리 연출이든, 그들 간의 역동적이고 매력적인 상호작용을 목격하십시오.
...
- **제한 없는 AI 소스 통합: 무한을 포용하다**
  fount는 AI 소스 연결에 있어 타의 추종을 불허하는 *선택*과 *유연성*을 제공합니다. AI 소스 생성기의 사용자 정의 JavaScript 코드를 사용하면 OpenAI, Claude, OpenRouter, NovelAI, Horde, Ooba, Tabby, Mistral 등 _어떤_ AI 소스에도 연결할 수 있습니다. 코드의 흐름 속에서 복잡한 정규 표현식을 신중하게 설계하고, 방대한 API 라이브러리를 호출하며, 멀티미디어 리소스를 삽입할 수 있습니다. fount는 또한 지능적인 요청 라우팅을 달성하기 위해 API 풀 생성을 기본적으로 지원합니다. 통신의 로직은 _당신의_ 의지에 따르며, 코드의 힘으로 형성됩니다.
  ![이미지](https://github.com/user-attachments/assets/f283d1de-c531-4b7a-bf43-3cbe0c48b7b9)

역할의 바다에서 길을 잃었거나 잊혀진 이야기를 찾고 있다면, [**활기차고 친근한 저희 커뮤니티**](https://discord.gg/GtR9Quzq2v)가 당신의 방문을 기다립니다. 이곳은 개발자와 창작자들이 그들의 지혜와 창작물을 공유하는 같은 생각을 가진 사람들의 안식처입니다.

---

## 동반자 관계: 디지털 장막 너머

fount는 캐릭터를 당신의 삶이라는 직물에 짜 넣으려 노력하며, 스크린을 넘어선 동반자 관계와 지원을 제공합니다.

- **Discord/Telegram 통합**
  내장된 봇 셸을 통해 당신의 캐릭터를 Discord나 Telegram 커뮤니티에 연결하여, 그들을 당신의 서클에 살아있는 멤버로 만들어보세요.
...
### 확장: 다양한 실로 엮어낸 혁신의 태피스트리

fount의 세계에서는 모듈성이 최고입니다. 풍부한 컴포넌트 생태계가 서로 얽혀 당신의 경험이라는 태피스트리를 만들어냅니다. 이 모든 것은 사용자가 손쉽게 설치, 확장 및 사용자화할 수 있습니다.
...
- **Discord 커뮤니티:** 실시간 지원과 토론을 위해 저희의 [활기찬 Discord 커뮤니티](https://discord.gg/GtR9Quzq2v)에 참여하세요.

당신의 목소리는 반드시 들릴 것입니다. fount를 다시 시작하기만 하면, 그림자는 사라질 것입니다.
...
- [**Google Chrome**](https://www.google.com/chrome/) / [**Microsoft Edge**](https://www.microsoft.com/edge/) / [**Opera**](https://www.opera.com/): 그들은 fount의 정신과 동기화되어 움직이며, 시처럼 부드러운 상호작용을 위해 최고의 성능과 호환성을 제공합니다.
...
- **Mozilla Firefox**: 그것은 고집 센 방랑자와 같습니다. 새로운 기술을 받아들이는 데 늘 느긋하며, 때로는 영원히 과거에 머무르기를 선택합니다. 하지만 이 고집은 몇 가지 안타까운 결과를 낳을 수 있습니다.
  ...
  - [CSS `anchor`](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_anchor_positioning) 포지셔닝 지원이 없어, 일부 페이지의 UI에 불완전한 먼지가 묻어 순수해야 할 분위기를 망칩니다.
...
AI의 속삭임 너머, fount는 더 깊은 연결을 제공합니다──*장인의 손길*. 우리 커뮤니티에는 미리 만들어진 캐릭터와 페르소나 템플릿이 풍부하며, _각각은 당신의 독특한 비전에 의해 깨어나기를 기다리는, 정성껏 조각된 기반입니다_.
