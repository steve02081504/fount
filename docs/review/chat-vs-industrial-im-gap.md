...
1. **消息手感缝**：语音是 `.wav` 附件（无按住说话/波形）；无用户定时发信；引用父消息不在当前页时常显示 `…`；无频道级 NSFW 进房门控。
2. **已读语义**：频道水位 + 已读人数角标（seq 推算），非 Signal 式单条已读回执。
...
Web Hub + PWA；Web Push 有；**无**原生 App / APNs / FCM（产品边界「明确不做」）。账号 = operator 密钥，无中心化「忘密码」。联系人发现（mDNS/Nostr/邀请码/`entities/search`）已有。多端 `read_marker` WS 有；无 QR link-device UI。
...
| 功能 | 现状 | 用户可见缺口 |
| --- | --- | --- |
| **语音消息** | 录音 → `.wav` 附件 + `<audio>` | 无按住说话 / 波形 / 转写（`voiceRing` 属通话 UI，勿混） |
| **定时发送** | **无**（social 有 `publishAt`；`timerTrigger` 是 char 定时回复） | 不能预约发信 |
| **内联 quote-reply** | 能回复、有引用条、可跳转加载 | 父消息不在当前页时预览常 `…`；无回复数角标 |
| **单条已读** | `member-read-markers` 用 seq 水位推算已读人数 | 无 per-message receipt |
| **频道 NSFW gate** | 消息级 CW / `sensitive_media` 有 | 无频道旗标 + 进房确认 |
| **富链接** | 前端 OG 水合 | 非结构化入库 |

已齐、勿当缺口：草稿（`composerDraft`）、离线队列（`sendQueue`）、投递态 pending→sent→delivered（`delivered` = 本机 WS 回显，非联邦证明）、`@mention` inbox、本地 sticker pack、屏幕共享。

形态不同：内联 `replyTo` **与** 子频道 thread drawer 双轨；慢速模式有 `messageRateLimitPerMin` 数字设置，无 Discord 式倒计时条。
...
| 引用预览 | `hub/messages/render/blocks.mjs`；`channelMessageStore.mjs` |
