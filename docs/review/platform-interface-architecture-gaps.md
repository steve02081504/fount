...
| M5 | telegrambot 遗留死代码：`format.mjs` 内 `TelegramMessageToFountChatLogEntry` / `telegramMediaGroupMessagesToFountChatLogEntry` 旧 chatLog 路径仍导出但全仓库无消费者（扁平 `default_interface.mjs` 副本已清理，此前记载过时）；`default_interface/` 目录未如规划「消失」——但 `*-api` 插件依赖其 `get*ForChar` 导出，保留属合理现实 | 死代码待删；「目录消失」的规划假设应作废 |
