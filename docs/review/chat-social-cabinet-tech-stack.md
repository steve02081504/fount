...
最后核对：`2026-07-18`（P0–P2 已落地）。对象：三壳实现债——设计别扭点、死代码、可合并架构。**不是**产品差距（产品见同目录 gap 档）。证据以 import 图与代码阅读为准。
...
| 优先级 | 动作 | 状态 |
| --- | --- | --- |
| P0 | 删 `fountMessageMarkdown*.mjs`、`mailboxApi.mjs`（零引用） | **已做** |
| P1 | 合并 `resolveSensitiveMedia` + 可信 Markdown + Social `authorLabel`→`resolveDisplayName` | **已做** |
| P2 | Chat WS 出站并入 `hub/stream/outbound.mjs`；`reloadChannel` 模块级绑定 | **已做** |
| P3 | 拆 Cabinet `public/index.mjs`（~1300 行）、Social `composer.mjs`（~530 行） | 待做 |
| P4 | ChatClient 按域拆方法工厂（对齐 SocialClient）；inbox 存储原语已有 `jsonlInboxStore` | 部分（inbox 已抽） |
...
**原状**：每个导出把 `loadMessages` / `syncCtx` 层层下传。

**现况**：`messages/messageContext.mjs` 导出 `reloadChannel` + 无参 `syncChannelActionsContext` / `bindReactions`；`messageRefresh` / `messageVirtualList` / `messageScroll` / `messageSend` 直接调用，不再套娃。

---

### 2. Chat 双 WebSocket 出站层 — **已修**

**原状**：`groupWsClient.mjs` 与 `stream/connection.mjs` 两套入口。

**现况**：出站在 `hub/stream/outbound.mjs`（`reportTyping` / `stopGeneration` / `sendWebsocketMessage`），读写同一 `connectionState.groupWebSocket`；`groupWsClient.mjs` 已删。
...
`fountMessageMarkdown*.mjs` / `mailboxApi.mjs` 已删。信任模型以 `hub/AGENTS.md` + `shared/trustedMarkdown.mjs` 为准。
...
| 路径 | 处置 |
| --- | --- |
| `fountMessageMarkdown.mjs` + `fountMessageMarkdownPlugins.mjs` | 已删 |
| `mailboxApi.mjs` | 已删 |
| `groupWsClient.mjs` | 已删；出站 → `hub/stream/outbound.mjs` |

### 已合并（不再双份维护）

| 逻辑 | 现况 |
| --- | --- |
| `resolveSensitiveMedia` | 唯一定义在 chat `messageFields.mjs`；Social `mediaRefs.mjs` 再导出 |
| 可信 Markdown | `chat/public/shared/trustedMarkdown.mjs`；`paintEntityProfileBio` + Social `display` 共用 |
| 展示名 | Social `authorLabel` → `resolveDisplayName` |
...
- Social `display.mjs` 的 `renderMarkdown` 与 `pages/scripts/features/markdown` 的同名函数**签名不同**（Social 为薄包装）
...
**D. Inbox 存储原语** — 已落地 `createJsonlInboxStore`；各自保留 `derive*Row` 与通知聚合。
...
| 壳 | 强 | 弱 |
| --- | --- | --- |
| **Chat** | 实体模型统一；Hub 已拆 `stream/`（含 outbound）+ `reloadChannel`；`shared/*` 跨壳复用；测试面大 | ChatClient 单文件膨胀；路由三处注册；view-log/raw 双读 |
| **Social** | SocialClient 组合式 API；`chatApi` 避免重复路由；展示名/Markdown/敏感媒体已对齐 chat shared | `composer.mjs` 上帝模块 |
