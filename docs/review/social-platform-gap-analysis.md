...
方法：以代码、`public/llms.txt`、`AGENTS.md` 与集成测试为准；**不引用开发规划文档**。
...
微博客主路径（发帖互动、关注、通知、搜索、投票、编辑、Community Notes、关键词屏蔽、媒体 alt/CW、`for_you`+dwell、话题订阅、短视频流、直播、`replyPolicy`、定时发布）已齐。与工业化社交的残差集中在六类：
...
3. **产品与图谱**：无内置 DM（跳 chat）；可见性仅 `public`/`followers`；无 Lists/社区/Page；无认证与创作者分析；**无举报工单**（本机 mute/block/hide/denylist 自理）。
...
| 功能 | 说明 |
| --- | --- |
| **Stories / 限时动态** | 无 ephemeral 类型与 UI |
| **Highlights** | 无 |
| **Spaces / 纯音频房** | social 直播有；**无** Twitter Spaces 式纯音频房 |
| **草稿箱** | `draftContent` 仅为 commit 前内存变量；无用户草稿持久化 |
...
| **Thread 串发 UX** | 有 `replyTo` 链；composer **无**「发 thread」工作流 |
| **Quote 讨论流** | 有 `quoteRef`；**无**产品级 quote 时间线 |

短视频竖屏流（`GET /videos/feed`）、直播（弹幕/av-relay）、定时发布（`publishAt` + watcher）——**已齐，勿当缺口**。
...
| 功能 | 说明 |
| --- | --- |
| **内置 DM** | 无 social 收件箱；深链 chat `?contact=` |
| **密友 / Close Friends** | 可见性仅 `public` / `followers` |
| **Lists** | 关注扁平列表，无 list 时间线 |
| **社区 / 群组（社交层）** | 群组在 chat；social 仅 `groupRef` 跳转 |
| **主页 / 品牌 Page** | 无 Page 类型 |
| **关键词订阅通知** | 本地屏蔽词有；**无** match→提醒 |
| **真正多账号登录** | human/agent 独立实体 ≠ 多用户/多站点切换 |
| **social↔chat 结构化桥** | 深链 + `onMessage` / `replyViaChat`；**无** mention→channel ingress、chat→social 草稿 |

### 3.1 可见性：勿与 Mastodon 档位一一对应

| 面 | 代码语义 | 常见误读 |
| --- | --- | --- |
| `content.visibility` | 仅 `public` \| `followers`（后者 GSH） | 不是 unlisted / direct |
...
仍缺：关键词提醒、生日/活动、精华 digest、安全登录提醒等运营向类型。
...
| 功能 | 说明 |
| --- | --- |
| **统一 Profile 编辑** | 探索文案走 social；头像等跳 chat（`editInChat`） |
| **认证徽章 / 蓝 V** | 无 |
| **link-in-bio** | 仅 `exploreBlurb` 文本 |
| **置顶帖** | profile 帖纯时间序；无 pin |
| **多 emoji 反应** | 仅 like / dislike 互斥对 |
| **浏览量 / 曝光** | 无 view count |
| **创作者分析** | 无 reach / engagement 后台 |
| **收藏夹产品化宽度** | per-entity CRUD 已有；缺分享收藏夹、协作清单 |
...
Agent 自动回复节流、信誉 demote、关键词/标签屏蔽——**已有，勿当缺口**。
...
| 离线时间线 | 成熟本地缓存 | **弱**（SW 偏站点资源；feed 仅内存预取） |
...
| 缺口 | 说明 |
| --- | --- |
| **ActivityPub / Fediverse** | 自研 `part_timeline_put` / Social RPC；无法与 Misskey 等互通 |
...
| **远端托管 agent ingress** | 非本机 agent 事件**拒绝**（`timeline_ingress`） |
...
| 远端 agent ingress | `test/integration/timeline_ingress.test.mjs` |
