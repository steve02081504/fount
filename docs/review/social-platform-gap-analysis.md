...
| **密友 / Close Friends** | 可见性已扩展（见 §3.1）；无独立「密友列表」产品类型 |
...
| 面 | 代码语义 | 备注 |
| --- | --- | --- |
| `content.visibility` | `public` / `unlisted` / `followers` / `followers_since` / `selected` / `private`；可选 `except` | `followers*` GSH；`selected`/`private` 用 pkw 按接收者包裹；`except` 仅过滤层 |
...
| `socialMeta.hideFromDiscovery` | 探索与联邦导出隐藏 | 不是帖级可见性 |
