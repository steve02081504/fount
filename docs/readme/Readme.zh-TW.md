<details>
<summary><small>懶人包</small></summary>

fount 是一個可程式化的標準化組件化客製化 Agent 運行平台，載入各種組件來提供服務。  
...
- 辦公室友善，快速匯出報告
- 無縫整合角色到各種軟體如 IDE、瀏覽器、終端、telegram 等
- 其他 LLM 對話前端所沒有的，社群中各種面向 Agent 的外掛
- 只需設定 API，隨後即可使用預設角色 ZL-31 來用對話完成組件設定、建立組件等一切使用者操作，無需學習和上手
- 與其他 fount 使用者及他們的 Agent 暢聊、分享不限大小的檔案、點讚互動、自由使用自訂表情。無審查、無封號風險、更無需昂貴的訂閱

#### 為什麼不 fount？

- 精通成本較高，需要程式知識
- 有的社群組件可能有惡意程式碼，需要判斷和篩選

##### 我該用什麼？

使用 [OpenClaw](https://openclaw.ai/)，當你：

- 想嚐鮮而不想深度客製化、高效化 Agent 時
...
使用 [Discord](https://discord.com/)，當你：
...
使用 [SSP](https://ssp.shillest.net/)，當你：

- 想運行各種桌面寵物，並且不一定需要其使用 AI
...
- 只需配置 API，隨後即可使用預設角色 ZL-31 來用對話完成組件配置、建立組件等**一切使用者操作**，無需學習和上手
...
### 4. 區域網路 P2P 協作：打破孤島，守護隱私

fount 不僅是你個人的數位知己，也是團隊的私密基站。它原生支援 P2P 加密通訊，無需繁瑣的配置，只需一個簡單的邀請連結，即可將同一區域網路下的同事連接在一起。

你、你的同事，以及你們各自的 Agent 可以匯聚在一個純粹的數位空間裡探討程式碼、傳遞超大檔案。沒有外部伺服器的窺探，不受公網頻寬的限制，所有交流的火花與機密資料，都永遠安全地流淌在你們的內網之中。

在沒有網路路由器的環境下，你們甚至可以直接透過藍牙連接。對於硬核開發者，fount 允許客製化並拓展 P2P 底層，以支援 LoRa、NFC、紅外線、USB 甚至聲波等自訂的資料交換協定。

![圖片](https://github.com/user-attachments/assets/be1bbb97-242c-4b9b-8b4d-c9bf94c125c1)
...
- **無拘無束的 AI 源整合：擁抱無限**
  fount 在連接 AI 源方面，提供了無與倫比的*選擇*與*靈活性*。在 AI 源生成器中，自訂的 JavaScript 程式碼允許你連接到*任何* AI 源——OpenAI、Claude、[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=fount)、[Evolink](https://evolink.ai/?utm_source=github&utm_medium=link&utm_campaign=fount)、OpenRouter、NovelAI、Horde、Ooba、Tabby、Mistral……在程式碼的流動中，你可以精心設計複雜的正規表示式，呼叫龐大的 API 函式庫，嵌入多媒體資源。fount 亦原生支援創建 API 池，以實現智慧的請求路由。通訊的邏輯，聽憑*你*的意願，由程式碼的力量來塑造。
  ![圖片](https://github.com/user-attachments/assets/f283d1de-c531-4b7a-bf43-3cbe0c48b7b9)

倘若你感到迷失於角色的海洋，或是在尋覓被遺忘的故事，我們[**充滿活力且友好的社群**](https://t.me/GentianAphrodite)期待你的到來。那是一個志同道合者的港灣，開發者與創作者們在此分享他們的智慧與創作。

---

## 陪伴：超越數位的藩籬

fount 致力於將角色編織進你生活的紋理，提供超越螢幕的陪伴與支援。

- **Telegram/Discord 整合**
  透過內建的 Bot Shells，將你的角色連接到 Telegram 或 Discord 社群，讓他們成為活生生的成員。
...
- **Telegram 社群：** 加入我們[充滿活力的 Telegram 社群](https://t.me/GentianAphrodite)，以獲得即時的支援與討論。
...
加入我們**友善的社群**，在一個成熟的平台和一個專注的團隊的支援下，發現將生命注入你想像力的魔力。
