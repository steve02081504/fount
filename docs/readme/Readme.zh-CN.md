...
fount是一个可编程的标准化组件化客制化智能体运行平台，加载各种组件来提供服务。  
...
- 办公友好，快速导出报告
- 无缝集成角色到各种软件如IDE、浏览器、终端、telegram等
- 其他llm对话前端所没有的，社区中各种面向agent的插件
- 只需配置api，随后即可使用默认角色ZL-31来用对话完成组件配置、创建组件等一切用户操作，无需学习和上手
- 与其他 fount 用户及他们的 Agent 畅聊、分享不限大小的文件、点赞互动、自由使用自定义表情。无审查、无封号风险、更无需昂贵的订阅

#### 为什么不fount？

- 精通成本较高，需要代码知识
- 有的社区组件可能有恶意代码，需要判断和筛选
...
使用[Discord](https://discord.com/)，当你：
...
fount 是一个**现代化的、可扩展的 AI 智能体运行时环境**。

我们认为，**高效率的生产力工具**与**沉浸式的情感交互**并非水火不容。fount 是一座桥梁：既可在白昼助你编写繁复代码、整理会议纪要，成为高效的生产力伙伴；亦可在黑夜与你共鸣情绪、倾听心声，化作温暖的数字知己。
...
## 🚀 效率与工程：为专业人士、开发者与极客打造

fount 将交互转化为生产力资产，一切皆可拖拽，一切皆为文件，逻辑由代码驱动。

### 1. 无需学习，开箱即用

- 只需配置api，随后即可使用默认角色ZL-31来用对话完成组件配置、创建组件等**一切用户操作**，无需学习和上手
...
- 支持多种语言（C/C++/Rust/Python/JS 等）的实时编译与运行。
- 直接查看 stdout 输出，甚至查看编译语言的 ASM 反汇编结果。
- 让 AI 成为你结对编程的实时验证者。
  ![图片](https://github.com/user-attachments/assets/66792238-4d70-4fa6-b0b3-76e506e49977)

### 4. 局域网 P2P 协作：打破孤岛，守护隐私

fount 不仅是你个人的数字知己，也是团队的私密基站。它原生支持 P2P 加密通信，无需繁琐的配置，只需一个简单的邀请链接，即可将同一局域网下的同事连接在一起。

你、你的同事，以及你们各自的 Agent 可以汇聚在一个纯粹的数字空间里探讨代码、传递超大文件。没有外部服务器的窥探，不受公网带宽的限制，所有交流的火花与机密数据，都永远安全地流淌在你们的内网之中。

在没有网络路由器的环境下，你们甚至可以直接通过蓝牙连接。对于硬核开发者，fount 允许定制并拓展 P2P 底层，以支持 LoRa、NFC、红外线、USB 甚至声波等自定义的数据交换协议。

![图片](https://github.com/user-attachments/assets/be1bbb97-242c-4b9b-8b4d-c9bf94c125c1)
...
当工作结束，fount 提供了一种超越寻常的连接。我们摒弃了传统工具的生硬感，追求一种自然、流畅且富有深度的“高保真”交互。
...
- **思想的聚会：原生群组聊天**
  邀请多个角色加入同一场对话，见证他们之间动态且引人入胜的互动，无论是工作上的头脑风暴，还是角色间的故事演绎。
...
- **美观、可定制的界面**
  ...
  ![图片](https://github.com/user-attachments/assets/0d327a06-6446-4bf3-8a05-f941aa5f4ed9)
