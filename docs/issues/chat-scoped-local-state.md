...
- `chat_scoped_char_memory` 的读写路径：
  - 读：`src/public/parts/shells/chat/src/chat/session/chatRequest.mjs` — `chat_scoped_char_memory: scopedState.memory`（引用注入 request）。
...
2. `chatRequest.mjs`：
   - `chat_scoped_char_memory` 改为 `await getScopedMemory(username, groupId, channelId, request.char_id)`（本地状态不存在时直接用空对象 `{}`；不兼容、不迁移）。
