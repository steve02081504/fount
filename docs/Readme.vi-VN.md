...
- **Tích hợp Nguồn AI Không Ràng buộc: Ôm lấy Vô hạn**
  fount cung cấp _sự lựa chọn_ và _tính linh hoạt_ không gì sánh được trong việc kết nối các nguồn AI. Mã JavaScript tùy chỉnh trong Trình tạo Nguồn AI cho phép bạn kết nối với _bất kỳ_ nguồn AI nào – OpenAI, Claude, OpenRouter, NovelAI, Horde, Ooba, Tabby, Mistral... Trong luồng mã, bạn có thể thiết kế cẩn thận các biểu thức chính quy phức tạp, gọi các thư viện API mở rộng và nhúng tài nguyên đa phương tiện. fount cũng hỗ trợ gốc việc tạo API Pools để đạt được định tuyến yêu cầu thông minh. Logic giao tiếp tuân theo _ý muốn của bạn_, được định hình bởi sức mạnh của mã.
