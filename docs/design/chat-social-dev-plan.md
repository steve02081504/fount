...
- **事件走数据，操作走对象**：char 的入站事件面是可序列化纯数据（上一条）；char 的**出站操作面**是 discord.js 式的 `ChatClient` 对象模型（M4）——群、频道、消息、成员、角色、反应全部建成 JS 对象，agent、agent 开发者与 AI 运行时共用。同一鸭子类型契约覆盖 fount 原生群与桥接群：平台对接 = 照契约实现一个翻译层（`bridgeOps`），不是另发明一套 API。操作平权（操作平权审阅 O3–O5）由此落地——对象方法以 acting entity 归因代签，权限即成员权限。
...
//            bridge?: { platform, platformChatId },  // M5 桥接群标记
//            memberCount },                 // active 成员数
//   channel: { channelId, name, kind: 'text'|'thread' },
// }
```

这层抽象同时是 Discord guild/channel、Telegram chat/thread 的公共投影——「有绑定对端的群即为 DM」，char 端 DM 判定统一为 `group.kind === 'dm'`，不再需要 per-platform 特判或 `onlineCount` 这种伪信号（要人数用 `group.memberCount`）。它也是 M4 `Group` / `Channel` 对象的**可序列化投影**：事件里只带这份纯数据，char 需要操作时经 `ChatClient` 水合成带方法的对象。
...
- **语义**：
  ...
  - agent：care 不改变触发——`onMessage` 一律送达；care 只是 `isCaredBy` 可查的事实（龙胆的「主人」= 龙胆 care 列表成员，见 M7）。
...
## M4 — ChatClient 对象模型（chat 操作面平权）

discord.js 式的面向对象操作层：群、频道、消息、成员、角色建成 JS 对象，写方法落到既有 DAG 原语，读方法是物化 state / view-log 的薄封装。三类消费者共用一套契约：

1. **agent / agent 开发者**：char 代码 import 后面向对象操作 chat——替代「读 llms.txt 拼 HTTP」与龙胆式自研 `PlatformAPI_t`。
2. **AI 运行时**：对象直接注入 code_execution 上下文（`fount.*` + 平台 `discord.*`/`telegram.*`/`wechat.*` 命名空间），AI 写 `await fount.message.react(':[g/e]')` 或 `await discord.channel.send(...)` 即可——龙胆旧 `interfaces/*/api.mjs` 的裸客户端形态收编为跨平台统一 + 具名命名空间。
3. **平台桥接壳**：TG / DC / WeChat 按同一鸭子类型实现「平台侧才有意义」的操作子集（`bridgeOps`），对象层查表转发——平台对接 = 照契约实现一个翻译层。

### 现状锚点（调研）

- 龙胆 `interfaces/{discord,telegram}/platform-api.mjs` 各自实现 `bot_core/index.mjs` 的 `PlatformAPI_t`：`sendMessage` / `sendTyping` / `fetchChannelHistory` / `getGroupMembers` / `generateInviteLink` / `leaveGroup` / `getGroupDefaultChannel` / `getOwnerPresenceInGroups` / `sendDirectMessageToOwner` / `onGroupJoin` / `onOwnerLeaveGroup`——这是「agent 实际需要的群操作面」的实战清单，本对象模型必须全覆盖（映射表见 M7）。
- 龙胆 `interfaces/{discord,telegram}/api.mjs` 经 code_execution 暴露平台原生客户端对象给 AI（关键词门控提示 + `GetJSCodeContext`）——证明「给 AI 对象而非 HTTP」是有效交互形态，但目前 per-platform 各写一份，fount 原生群反而没有。
- chat shell 写原语齐全但仅 HTTP + operator 形态：`dag/channelOps.mjs`（频道/反应/置顶/文件）、governance 路由的 `appendSignedLocalEvent`（踢/禁/角色）、`dm/index.mjs`（ECDH DM）、`lib/inviteTickets.mjs`；操作平权审阅 2.2–2.5 节矩阵中 agent 列几乎全 ❌。

### 4.1 actor 委托（写路径地基）

对齐操作平权审阅 §6.1–6.2，把 social 的 `resolveActingEntity` 委托写模型带进 chat：

- **新建** `src/chat/lib/actor.mjs`：

```js
export async function resolveChatActor(username, actingEntityHash)
// → { kind: 'user'|'agent', entityHash, charname? }
// 缺省 = operator；指定值须为 operator 或本机托管 agent（校验对齐 1.1 inbox recipient），否则 throw
```

- `dag/append.mjs` 增加 `appendActorEvent(username, groupId, actor, body)`：user actor 即现 `appendSignedLocalEvent`；agent actor → 该 agent 成员行的 `ownerPubKeyHash` 对应 signer 代签（`sender = ownerPubKeyHash`），`content.actingAgentEntityHash` 归因，权限按 **agent 成员行**的 roles 算（`calculateMemberPermissions` 传 agent 成员）。
- `authorizeEvent.mjs` 联邦侧同规则确定性复核：带 `actingAgentEntityHash` 的事件，sender 必须等于该 agent 成员行的 `ownerPubKeyHash` 且权限查 agent 成员行——远端节点照旧全量校验，委托只在信任域内放宽「谁能发起」，不放宽「谁有权做」。
- 消息发送走 `messageCommit` `origin: 'char'`（既有 char 归因路径，charId / displaySnapshot 复用）。
- **有意不对等**（对齐操作平权审阅 O0，写进 `llms.txt`）：agent 不能建群、不能持群 ownership、不能独立持钥——`client.createGroup` 对 agent actor 直接 throw。

### 4.2 对象模型（新建 `src/public/parts/shells/chat/src/api/`）

纯鸭子类型，无继承要求；对象由 id 惰性水合，方法内部全部复用既有函数——本层**不新增任何存储与事件类型**：

```js
export async function getChatClient(username, actingEntityHash)  // → ChatClient
```

| 对象 | 属性 | 方法（→ 内部落点） |
| --- | --- | --- |
| `ChatClient` | `entityHash` | `groups()`、`group(id)`、`openDm(entityHash)`（→ `dm/index.mjs`，agent↔operator 亦可）、`createGroup(opts)`（user actor 限定 → `crud.mjs::newGroup`）、`messageFrom(event)`（把 `onMessage` 纯数据事件水合成 `Message`） |
| `Group` | `id, name, kind, memberCount, bridge?`（= 1.4 投影） | `channels()` / `channel(id)` / `defaultChannel()`、`members({ page })` / `member(entityHash)`（→ 物化 `state.members`）、`roles()` / `role(id)`、`createChannel(opts)`（→ `channelOps`）、`createInvite()`（→ `mintGroupInviteTicket`，返回深链文本）、`leave()`（→ `member_leave` + replica 清理）、`setMeta(patch)`（→ `group_meta_update`） |
| `Channel` | `id, name, kind` | `send(reply)`（→ `messageCommit`，返回 `Message`）、`typing()`（原生：WS `typing` 广播；桥接：`bridgeOps.sendTyping`）、`messages({ limit, before })`（→ viewer=actor 的 view-log 语义，含可见性 ACL）、`pins()`、`startVote(ballot)` |
| `Message` | `eventId, channelId, content, files, mentions, time, author → Member` | `reply(reply)`、`edit(patch)` / `delete()`（→ `message_edit` / `message_delete`，authorize 管归属）、`react(emoji)` / `unreact(emoji)`（→ `appendReactionEvent`）、`pin()` / `unpin()`、`mentionsEntity(hash)`（→ 1.4 `messageMentionsEntity` 薄封装） |
| `Member` | `entityHash, memberKind, displayName, roles, joinedAt` | `kick()` / `ban()` / `unban()`（→ `member_kick` / `member_ban` / `member_unban`）、`addRole(roleId)` / `removeRole(roleId)`（→ `role_assign` / `role_revoke`）、`dm()`（= `client.openDm`） |
| `Role` | `id, name, permissions, position` | `members()`（物化索引查询；mention 结构永不物化成员集合的不变量只约束 fanout，这里是显式管理查询） |

- 全部写方法 = `appendActorEvent` 封装：权限不足由 authorize 拒绝直接冒泡，**不做防御性预检**；调用方传垃圾直接炸是调用方的问题。
- `onMessage` 事件形状不变（M1，可序列化）；「事件给事实，判断走 import」原则不变——对象模型就是那组 import 函数的组织形态，`client.messageFrom(event)` 是 char 侧从事实到操作的唯一桥。
- 进程内 API，不另起 HTTP 面：Hub 前端继续走既有路由；plugin / world / persona 代码同样可 import（world 的 `WorldChatHost` 保持既有特权面不动）。
- 远端托管 agent 的 client 方法 RPC 化列入「后续方向」，本周期只服务本机 parts。

### 4.3 code_execution 注入

- **新建** `src/chat/lib/codeContextPlugin.mjs`：chat 触发生成时（`chatRequest.mjs` 组装 plugins 处）注入内建 `fount_chat` 插件——`GetJSCodeContext` 返回 `{ fount: { chat, group, channel, message? } }`（当前会话已水合对象，client 以该 char 为 acting entity）；`GetJSCodePrompt` 简述 `fount.*` 形态与权限语义。平台 API 插件（discord-api / telegram-api / wechat-api）各自注入 `{ discord }` / `{ telegram }` / `{ wechat }` 命名空间。
- 龙胆 `api.mjs` 的关键词门控（决定要不要往 prompt 里塞提示）属于 char 的策略，留在龙胆侧；上下文供给收归 shell。

### 4.4 bridgeOps（桥接鸭子类型）

**新建** `src/chat/bridge/ops.mjs`：

```js
export function registerBridgeOps(platform, ops)   // bot 壳启动时注册（M5/M6）
export function resolveBridgeOps(platform)
// ops 形状（全部可选）：
// { sendTyping({ platformChatId, platformThreadId }),
//   kickMember({ platformChatId, platformUserId }), banMember(同), unbanMember(同),
//   createInvite({ platformChatId }) → url,
//   leaveChat({ platformChatId }),
//   openDm({ platformUserId }) → { platformChatId },
//   fetchHistory({ platformChatId, platformThreadId?, limit }) → dto[],   // 桥接群补齐 DAG 之外的平台历史
//   getNativeContext({ platformChatId, platformMessageId? }) → object }   // 平台原生对象逃生舱
```

对象层分派规则（`group.bridge` 存在时）：

- **纯 fount 概念**（反应、置顶、投票、fount 角色、view-log）照走 DAG，与原生群无差别。
- **平台侧才有意义**的操作（踢/禁平台用户、平台邀请链接、typing、平台 DM）查 `bridgeOps` 转发，成员的 `platformUserId` 经 M5 identity 反查表得出；两侧都要的（如 kick）先平台后 DAG（平台失败即 throw，不半做）。
- 未注册对应 op → throw；缺省不静默降级。
- `getNativeContext` 由 bot 壳注册、各 `*-api` 插件在 `GetJSCodeContext` 时消费（经 `codeBridgeContext.mjs` 反查 bridge 映射 + 触发消息 `extension.bridge.platformMessageId` 水合）。**不**向沙箱注入 `bridgeOps`；agent 侧逃生舱为 `discord.*` / `telegram.*` / `wechat.*` 命名空间内的原生字段（如 `discord.message`、`telegram.chatId`）。

### 验收

- 新集成测试 `chat/test/integration/chat_client_api.test.mjs`：agent actor 经 client 在原生群 `channel.send`（view-log 归因该 char）、`message.react` / `pin`（赋 `PIN_MESSAGES` 后成功，无权限 throw）、`group.members` 分页、有 `KICK_MEMBERS` 的 agent `member.kick` 用户成员、`openDm` 后 `send`。
- 联邦确定性：带 `actingAgentEntityHash` 的事件在第二节点 authorize 通过；`sender ≠` agent 成员行 `ownerPubKeyHash` 的伪造事件被拒。
- bridgeOps：mock 平台注册后，桥接群 `group.leave()` / `member.kick()` / `channel.typing()` 打到 mock 且 kick 落 `member_kick` DAG 事件；未注册 op 调用 throw。
- code_execution fixture char 断言上下文含 `fount.chat` / `fount.group` / `fount.channel` / `fount.message` 对象且 acting 归因正确。

---

## M5 — Bridge ingress + Telegram 壳改薄
...
### 5.1 chat bridge 层（新建 `src/public/parts/shells/chat/src/chat/bridge/`）
...
- **bridgeOps 注册**（M4.4 契约的 TG 实现）：`registerBridgeOps('telegram', { sendTyping, kickMember: banChatMember, unbanMember, createInvite: exportChatInviteLink, leaveChat, openDm, getNativeContext })`——即龙胆 `telegram/platform-api.mjs` 对应能力的官方归宿；TG Bot API 不支持的（如全量成员列举）不注册，对象层调用即 throw。
...
- bridgeOps：mock Telegraf 下 `channel.typing()` / `group.createInvite()` / `member.kick()` 经注册的 ops 打到平台调用。
...
## M6 — Discord / WeChat 同改 + 平台格式钩子

### 6.1 壳改造

- `discordbot` / `telegrambot` / `wechatbot` 均使用 `src/default_interface/main.mjs`（共享 `chat/bridge/interfaceKit.mjs` 的 ingest/outbound 骨架）；`format.mjs` 留在各壳 `src/`。
- `discordbot`：`MessageCreate/Update/Delete` → DTO → bridge；删除旧内存队列与直接 `GetReply`。
- `wechatbot`：长轮询 update → DTO → bridge。
- 三壳 `runBot` 保持「一个 bot 实例绑定一个 char」；bridge 群名默认取平台会话标题。
- **bridgeOps 注册**：各壳 `OnceClientReady`/`BotSetup` 内 `registerBridgeOps(platform, …)`；`getNativeContext` 供 `*-api` 插件水合，不注入 agent 沙箱。
...
| `interfaces/telegram/{event-handlers,message-converter,state,world}.mjs`、`interfaces/discord/` 同名 | **删除**——bridge 负责转换与收发 |
| `interfaces/{telegram,discord}/platform-api.mjs`（`PlatformAPI_t` 实现） | **删除**——能力全量由 M4 `ChatClient` 承接：`sendMessage`→`channel.send`；`sendTyping`→`channel.typing`；`fetchChannelHistory`→`channel.messages`（桥接群另有 `bridgeOps.fetchHistory` 补平台历史）；`getGroupMembers`→`group.members`；`generateInviteLink`→`group.createInvite`；`leaveGroup`→`group.leave`；`getGroupDefaultChannel`→`group.defaultChannel`；`sendDirectMessageToOwner`→`client.openDm(主人hash).send`；`getOwnerPresenceInGroups`→龙胆侧组合查询（`client.groups()` × `group.member(主人各身份 hash)`，归属查询本地 O(1)）；`onGroupJoin`/`onOwnerLeaveGroup`→入站事件面只有 `onMessage`，主人在场检查改为 `onMessage` 时惰性查询（连续 N 条消息主人不在场再走「辱骂退群」流程；无独立生命周期 hook 属有意收缩） |
| `interfaces/{telegram,discord}/api.mjs`（code_execution 暴露裸平台客户端） | **删除**——M4.3 shell 注入统一 `chat`/`group`/`channel`/`message` 对象；桥接群另有 `native` 逃生舱（`bridgeOps.getNativeContext`）覆盖平台长尾操作；龙胆的关键词门控提示策略并入其 prompt 逻辑 |
| `interfaces/telegram/utils.mjs` 贴纸 / HTML 出站定制 | `interfaces.telegram.FormatOutboundReply`（M6 钩子），仅保留与壳层默认实现不同的部分 |
| `telegram.BotSetup` / `discord.OnceClientReady` | 删除或收缩为空（连接由壳层默认管理；无自定义连接需求即整个删除） |
| `bot_core/{index,reply,state,utils,group_handler,error}.mjs` | **删除**（`group_handler` 的入群主人检查按上行 `getOwnerPresenceInGroups` 映射迁入 `onMessage`） |
...
- follower 分发只剩人类链：follower 为 operator entity → 人类通知链（inbox `follow` / feed）；agent 侧无需专门分发——被关注者新帖经 following 同步入账时已走 M8 的 `dispatchSocialMessage` → `onMessage`。
...
graph LR
  ...
  M1 --> M3[M3 具名层]
  M1 --> M4[M4 ChatClient 对象模型]
  M4 --> M5[M5 bridge ingress + TG]
  M5 --> M6[M6 DC/WeChat + 格式钩子]
  M6 --> M7[M7 龙胆迁移]
  M1 --> M8[M8 social onMessage + care 分发]
  M2 --> M8
  M9[M9 social actor 平权]
  M9 --> M10[M10 social 产品补强]
```

M4 依赖 M1 的 conversationContext / mentions 事实层（对象是其水合形态）；M5 依赖 M4 的 bridgeOps 契约；M8 依赖 M1 的事件形状约定与 M2 的 care / notify 基建；M9 与 M1–M8 无强依赖可并行（M9 与 M8 都动 `dispatch.mjs`，先后合并即可）；M10 的 `poll_closed` 通知复用 M2 的 notify 基建（`notifyUser`）。
...
- M4 以操作平权审阅第二节能力矩阵为蓝本：矩阵中 agent 列的每个 ❌ 项对应一条 client 用例（有权限成功 + 无权限拒绝 + 联邦第二节点 authorize 一致）。
- M5–M7 平台 API 一律 mock（synthetic DTO + mock bridgeOps），不依赖真实 TG / DC / WeChat 凭据；龙胆三端一致性回归随龙胆目录测试维护。
...
- **远端托管 agent 的 ChatClient**：M4 只服务本机 parts。跨节点 agent 的读方法天然可用（联邦群各节点持物化 state 副本），写方法需解决「代签者归属」（其 home 节点上该群 replica 的 signer 与 agent 成员行 `ownerPubKeyHash` 的关系），随「远端 agent 接纳」一并设计。
