...
2. **AI 运行时**：对象直接注入 code_execution 上下文，AI 写 `await message.react(':[g/e]')` 即可——龙胆 `interfaces/*/api.mjs` 把裸 `discord_client` / `telegram_client` 喂给 AI 做禁言/踢人/身份组操作的形态，收编为跨平台统一的官方形态。
...
- **新建** `src/chat/lib/codeContextPlugin.mjs`：chat 触发生成时（`chatRequest.mjs` 组装 plugins 处）注入内建 `fount_chat` 插件——`GetJSCodeContext` 返回 `{ chat: client, group, channel, message }`（当前会话已水合对象，client 以该 char 为 acting entity）；`GetJSCodePrompt` 简述对象形态与权限语义。
...
- `getNativeContext` 是逃生舱：桥接群的 code_execution 上下文额外挂 `native`（discord.js `Message` / telegraf ctx 等），覆盖 fount 契约没建模的长尾平台操作——龙胆迁移后高级操作能力不缩水。
...
- code_execution fixture char 断言上下文含 `chat` / `group` / `channel` / `message` 对象且 acting 归因正确。
...
- `discordbot/src/default_interface/` 同 M5 模式重写：`MessageCreate/Update/Delete` → DTO → bridge；删除 `ChannelMessageQueues` / `MargeChatLog` / 历史 fetch（DAG 即历史）；`tools.mjs` → `discordbot/src/format.mjs`（`splitDiscordReply`、`getMessageFullContent`）。
...
- **bridgeOps 注册**：`registerBridgeOps('discord', ...)` 全量实现（DC API 能力最全：kick/ban、invite、typing、DM、`getNativeContext` 给 discord.js `Message`/`Guild`）；`registerBridgeOps('wechat', ...)` 按平台能力注册子集。
