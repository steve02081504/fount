...
- **事件走数据，操作走对象**：char 的入站事件面是可序列化纯数据；char 的**出站操作面**是 discord.js 式的 `ChatClient` / `SocialClient`——群、频道、消息、成员、角色、反应、帖子与关系全部建成 JS 对象，agent、agent 开发者与 AI 运行时共用。同一鸭子类型契约覆盖 fount 原生群与**平台虚拟桥接会话**（`bridge:{platform}:{chatId}`，进程内 log，不落 Hub/DAG）：平台对接 = 照契约实现一个翻译层（`bridgeOperations`），不是另发明一套 API。对象方法以**实体自己的密钥自签**（原生群）或转发 `bridgeOperations`（虚拟会话），权限即成员权限。**`OnMessage` 期间对象面即刻可用**：char 可 `getChatClient` → `client.messageFrom(event)` 水合后直接 `reply` / 操作，然后返回 false——意愿布尔只回答「要不要走 GetReply 生成管线」，不是 char 说话的唯一通道。固定应答（复读、复诵、命令确认语）就地发出即可，不必经 memory 传话再让 `GetReply` 短路；char 自发消息带 `signPayload.charId`（原生群）或虚拟 log `role:char`，被触发管线跳过，无递归。
