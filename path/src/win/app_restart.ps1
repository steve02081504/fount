...
function script:Register-FountApplicationRestart {
	...
	$restartCommandLine = " -NoProfile -ExecutionPolicy Bypass -Command `".{ `$env:FOUNT_CLICK = 1; .\`"$FOUNT_DIR/path/fount.ps1\`"$restartArgs }`""
	[FountRestart]::RegisterApplicationRestart($restartCommandLine, 3) | Out-Null
}
