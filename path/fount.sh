...
update_fount_if_not_noupdate() {
	if [ -f "$FOUNT_DIR/.noupdate" ]; then
		echo "$(get_i18n 'update.skippingFountUpdate')"
	else
		fount_upgrade
	fi
}
...
upgrade_deno_if_not_docker() {
	if [ $IN_DOCKER -eq 1 ]; then
		echo "$(get_i18n 'update.skippingDenoUpgradeDocker')"
	else
		deno_upgrade
	fi
}
...
EOF
		...
		echo "$(get_i18n 'shortcut.protocolHandlerRegistered')"
	...
	if [ ! -d "$FOUNT_DIR/.git" ]; then
		...
		if [ "$localCommit" != "$remoteCommit" ]; then
			if [ "$mergeBase" = "$localCommit" ]; then
				echo "$(get_i18n 'git.updatingFromRemote')"
				git -C "$FOUNT_DIR" reset --hard "$remoteBranch"
			elif [ "$mergeBase" = "$remoteCommit" ]; then
				echo "$(get_i18n 'git.localBranchAhead')"
			else
				echo "$(get_i18n 'git.branchesDiverged')"
				git -C "$FOUNT_DIR" reset --hard "$remoteBranch"
			fi
		else
			echo "$(get_i18n 'git.alreadyUpToDate')"
		fi
	fi
}
...
install_deno() {
	...
	if [[ $IN_TERMUX -eq 1 ]]; then
		echo "$(get_i18n 'deno.installingTermux')"
		set -e
		...
		echo "$(get_i18n 'deno.missing')"
		if ! curl -fsSL https://deno.land/install.sh | sh -s -- -y; then
			echo "$(get_i18n 'deno.installFailedFallback')"
...
deno_upgrade() {
	...
	if ! run_deno upgrade -q "$deno_upgrade_channel"; then
		if [[ $IN_TERMUX -eq 1 ]]; then
			echo "$(get_i18n 'deno.upgradeFailedTermux')"
			rm -rf "$HOME/.deno"
			install_deno
		else
...
if [[ ! -d "$FOUNT_DIR/node_modules" || ($# -gt 0 && $1 = 'init') ]]; then
	...
	echo "$(get_i18n 'install.installingDependencies')"
...
clean)
	if [ -d "$FOUNT_DIR/node_modules" ]; then
		run shutdown
		echo "$(get_i18n 'clean.removingCaches')"
		rm -rf "$FOUNT_DIR/node_modules"
		find "$FOUNT_DIR" -name "*_cache.json" -type f -delete
	fi
	echo "$(get_i18n 'clean.reinstallingDependencies')"
	run shutdown
	echo "$(get_i18n 'clean.cleaningDenoCaches')"
	run_deno clean
	exit 0
	;;
keepalive)
...
remove)
	run shutdown
	run_deno clean
	echo "$(get_i18n 'remove.removingFount')"

	echo "$(get_i18n 'remove.removingFountFromPath')"
	...
	echo "$(get_i18n 'remove.removingProtocolHandler')"
	remove_desktop_shortcut

	echo "$(get_i18n 'remove.removingFountFromGitSafeDir')"
	...
	echo "$(get_i18n 'remove.removingInstalledSystemPackages')"
	if [[ $IN_TERMUX -eq 1 ]]; then
	...
	if [ -f "$AUTO_INSTALLED_DENO_FLAG" ]; then
		echo "$(get_i18n 'remove.uninstallingDeno')"
		rm -rf "$HOME/.deno"
		for profile_file in $(get_profile_files); do
	...
	echo "$(get_i18n 'remove.removingFountInstallationDir')"
	rm -rf "$FOUNT_DIR"
	# 只要父目录为空，继续删他妈的
	...
	echo "$(get_i18n 'remove.fountInstallationDirRemoved')"

	echo "$(get_i18n 'remove.fountUninstallationComplete')"
	exit 0
	;;
*)
	runargs=("${@}")
