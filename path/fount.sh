...
run() {
	...
	if [[ $# -gt 0 && $1 = 'debug' ]]; then
		run_deno run --allow-scripts --allow-all --inspect-brk --node-modules-dir=none "$FOUNT_DIR/src/server/index.mjs" "${@:2}"
	else
		run_deno run --allow-scripts --allow-all --node-modules-dir=none "$FOUNT_DIR/src/server/index.mjs" "$@"
	fi
	local exit_code=$?
...
if [[ ! -d "$FOUNT_DIR/node_modules" || ($# -gt 0 && $1 = 'init') ]]; then
	...
	run_deno install --reload --allow-scripts --allow-all --node-modules-dir=none --entrypoint "$FOUNT_DIR/src/server/index.mjs"
