...
Windows `bootstrap_full` writes `$FOUNT_DIR/fount.exe` when missing (gitignored) — Steam convenience, not required to run. `New-FountExe` keeps try/catch glued to `ps12exe`: any throw is reported ([ps12exe#58](https://github.com/steve02081504/ps12exe/issues/58)) and **not** rethrown. `geneexe` fails via `index.ps1` (`$Error.Count` / `$LastExitCode`). `Install-FountRootExe` only restores `$Error` / `LastExitCode` so `fount server` cannot fail — do not move the report catch there. path-cmd-smoke sets `GH_TOKEN`. If the favicon.ico file is missing, geneexe calls `run shutdown` so init compiles the icon. Call the `run` function, not `fount shutdown`, so bootstrap cannot recurse into `Install-FountRootExe`. `fount geneexe [path]` still defaults to `./fount.exe` (cwd).
