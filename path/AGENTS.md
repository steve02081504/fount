...
- **DB refresh throttle**: `pkg_db_refresh_needed`/`pkg_db_refresh_mark` — a manager's DB refresh (`pacman -Syy`/`-Sy`, `apt-get update`, `dnf makecache`, …) runs at most once per `FOUNT_PKG_REFRESH_INTERVAL` (default 600s), checked inside the lock. Removed the Arch-only "never refresh" and pacman skip paths.
