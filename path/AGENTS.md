...
- On Linux pacman hosts (excluding Termux), Deno updates never invoke system package upgrades. A pacman-owned executable is left to a full system upgrade; use a user-managed Deno for repository pins/channels. Other platforms and package managers retain their existing behavior. Arch dependency installs use the existing sync database, never refresh it for a partial upgrade.
...
- Path CLI: `deno_upgrade` (keepalive auto-repair + `fount test`) upgrades to the pinned spec, overriding the default channel; pacman-owned Deno on Linux is excluded as above.
