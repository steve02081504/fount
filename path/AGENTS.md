...
- `fount update pr/<n>` (also `pull/<n>`, `#<n>`, or a GitHub `…/pull/<n>` URL) fetches `refs/pull/<n>/head` into `origin/pr/<n>`, detaches there, and creates `.noupdate`. Re-run the same command to refresh the tip.
