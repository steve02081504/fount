...
- On pacman hosts, Deno updates never invoke system package upgrades. A pacman-owned executable is left to a full system upgrade; use a user-managed Deno for repository pins/channels. Other package managers retain their existing update behavior. Pacman dependency installs use the existing sync database, never refresh it for a partial upgrade.
...
- Path CLI: `deno_upgrade` (keepalive auto-repair + `fount test`) upgrades user-managed Deno to the pinned spec, overriding the default channel.
