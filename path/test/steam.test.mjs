...
Deno.test('applyFountSteamEntry adds then updates and drops extras', async () => {
	const { root, fountDir, user } = await makeSteamHarness()
	try {
...
Deno.test('ensureSteamImages writes pngs and skips when fresh', async () => {
	const { root, fountDir } = await makeSteamHarness()
	try {
		const imgs = join(fountDir, 'imgs')
...
Deno.test('findSteamInstallPaths uses the Windows registry callback', async () => {
	const { root } = await makeSteamHarness()
	try {
...
Deno.test('registerFountSteam reports need_exe on Windows when fount.exe is missing', async () => {
	const { root, fountDir, user } = await makeSteamHarness()
	try {
		const result = await registerFountSteam({
			fountDir,
...
Deno.test('register then unregister writes and clears shortcuts.vdf plus grid images', async () => {
	const { root, fountDir, user } = await makeSteamHarness()
	try {
...
Deno.test('copySteamImages replaces previous hero/logo/icon files', async () => {
	const { root, fountDir, user } = await makeSteamHarness()
	try {
