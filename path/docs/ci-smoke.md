...
2. `run-smoke.{sh,ps1}` exercises `server` / `background` / `log` / `reboot` / `init`. Nonzero `fount` dumps captured stdout/stderr (bash `set -e` + `$(…)` otherwise swallows it). Job env `GH_TOKEN` (`secrets.GH_TOKEN` else `GITHUB_TOKEN`) lets Windows `New-FountExe` (Steam shortcut path during `init`) auto-report a ps12exe throw.
